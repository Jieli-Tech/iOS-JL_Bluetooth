...
-(BOOL)installWithDevice:(JL_EntityM *)entity{
    ...
    NSLog(@"%s:%d name:%@,protocol:%d",__func__,entity.mType,entity.mItem,entity.mProtocolType);
...
-(BOOL)updateDeviceByObjc:(DeviceObjc *)entity{
    ...
    NSLog(@"%s:%d name:%@,protocol:%d",__func__,entity.type,entity.name,entity.mProtocolType);
...
-(void)checkWetherShouledNewOne{
    ...
    if (![db columnExists:@"protocolType" inTableWithName:@"t_devices"]) {
        ...
        BOOL worked = [db executeUpdate:alertStr];
        if(worked){
            NSLog(@"protocolType 插入成功");
        }else{
            NSLog(@"protocolType 插入失败");
        }
    }
