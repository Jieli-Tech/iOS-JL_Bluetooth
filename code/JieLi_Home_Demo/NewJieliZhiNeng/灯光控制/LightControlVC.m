...
-(void)setupUI{
    ...
    titleName.text= kJL_TXT("multi_media_light_settings");
    ...
    [btn_1 setTitle:kJL_TXT("light_coloured_lights") forState:UIControlStateNormal];
    [btn_2 setTitle:kJL_TXT("light_twinkle") forState:UIControlStateNormal];
    [btn_3 setTitle:kJL_TXT("light_scene") forState:UIControlStateNormal];
    ...
    [lightView_0 setBackgroundColor:kDF_RGBA(255, 255, 255, 1.0)];
    ...
    if(sH>=896){
        colorPicker.center = CGPointMake(sW/2.0, sH/3-35);
    }else{
        colorPicker.center = CGPointMake(sW/2.0, kJL_HeightNavBar+42+30+34);
    }
    ...
    if(sW<=375.0){
        if(sW == 320.0){
            firstView = [[UIView alloc] initWithFrame:CGRectMake(16, colorPicker.frame.origin.y+colorPicker.frame.size.height+15, [UIScreen mainScreen].bounds.size.width-16*2, 32)];
            secView = [[UIView alloc] initWithFrame:CGRectMake(16, colorPicker.frame.origin.y+colorPicker.frame.size.height+60, [UIScreen mainScreen].bounds.size.width-16*2, 32)];
        }else{
            firstView = [[UIView alloc] initWithFrame:CGRectMake(16, colorPicker.frame.origin.y+colorPicker.frame.size.height+40.5, [UIScreen mainScreen].bounds.size.width-16*2, 32)];
            secView = [[UIView alloc] initWithFrame:CGRectMake(16, colorPicker.frame.origin.y+colorPicker.frame.size.height+85, [UIScreen mainScreen].bounds.size.width-16*2, 32)];
        }
    }else{
        firstView = [[UIView alloc] initWithFrame:CGRectMake(32, colorPicker.frame.origin.y+colorPicker.frame.size.height+40.5, [UIScreen mainScreen].bounds.size.width-32*2, 32)];
        secView = [[UIView alloc] initWithFrame:CGRectMake(32, colorPicker.frame.origin.y+colorPicker.frame.size.height+85, [UIScreen mainScreen].bounds.size.width-32*2, 32)];
    }
    ...
    //H
    if(sW == 320.0){
        sliderSewen_0.frame = CGRectMake(42.5,secView.frame.origin.y+secView.frame.size.height+19, [UIScreen mainScreen].bounds.size.width - 2*47.5, 30);
        if([kJL_GET hasPrefix:@"zh"]){
            coldLabel.frame = CGRectMake(22, secView.frame.origin.y+secView.frame.size.height+23, 25, 25);
            warmLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-44, secView.frame.origin.y+secView.frame.size.height+23, 25, 25);
        }else{
            coldLabel.frame = CGRectMake(10, secView.frame.origin.y+secView.frame.size.height+23, 50, 25);
            warmLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-50, secView.frame.origin.y+secView.frame.size.height+23, 50, 25);
        }
    }else{
        sliderSewen_0.frame = CGRectMake(42.5,secView.frame.origin.y+secView.frame.size.height+24, [UIScreen mainScreen].bounds.size.width - 2*47.5, 30);
        if([kJL_GET hasPrefix:@"zh"]){
            coldLabel.frame = CGRectMake(22, secView.frame.origin.y+secView.frame.size.height+28, 25, 25);
            warmLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-44, secView.frame.origin.y+secView.frame.size.height+28, 25, 25);
        }else{
            coldLabel.frame = CGRectMake(10, secView.frame.origin.y+secView.frame.size.height+28, 50, 25);
            warmLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-50, secView.frame.origin.y+secView.frame.size.height+28, 50, 25);
        }
    }
    [sliderSewen_0 setMinimumTrackTintColor:kColor_0000];
    
    coldLabel.contentMode = UIViewContentModeRight;
    coldLabel.attributedText = [self setHSLText:kJL_TXT("light_color_temperature_cold")];
    warmLabel.contentMode = UIViewContentModeLeft;
    warmLabel.attributedText = [self setHSLText:kJL_TXT("light_color_temperature_warm")];
    
    //S
    if(sW == 320.0){
        sliderSewen_1.frame = CGRectMake(42.5, warmLabel.frame.origin.y+warmLabel.frame.size.height+19,[UIScreen mainScreen].bounds.size.width - 2*47.5, 30);
        if([kJL_GET hasPrefix:@"zh"]){
            grayLabel.frame = CGRectMake(22, warmLabel.frame.origin.y+warmLabel.frame.size.height+23, 25, 25);
            brightLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-44, warmLabel.frame.origin.y+warmLabel.frame.size.height+23, 54, 25);
        }else{
            grayLabel.frame = CGRectMake(10, warmLabel.frame.origin.y+warmLabel.frame.size.height+23, 50, 25);
            brightLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-50, warmLabel.frame.origin.y+warmLabel.frame.size.height+23, 54, 25);
        }
    }else{
        sliderSewen_1.frame = CGRectMake(42.5, warmLabel.frame.origin.y+warmLabel.frame.size.height+24, [UIScreen mainScreen].bounds.size.width - 2*47.5, 30);
        if([kJL_GET hasPrefix:@"zh"]){
            grayLabel.frame = CGRectMake(22, warmLabel.frame.origin.y+warmLabel.frame.size.height+28, 25, 25);
            brightLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-44, warmLabel.frame.origin.y+warmLabel.frame.size.height+28, 54, 25);
        }else{
            grayLabel.frame = CGRectMake(10, warmLabel.frame.origin.y+warmLabel.frame.size.height+28, 50, 25);
            brightLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-50, warmLabel.frame.origin.y+warmLabel.frame.size.height+28, 54, 25);
        }
    }
    [sliderSewen_1 setMinimumTrackTintColor:kColor_0000];
    
    grayLabel.contentMode = UIViewContentModeRight;
    grayLabel.attributedText = [self setHSLText:kJL_TXT("light_color_temperature_gray")];
    brightLabel.contentMode = UIViewContentModeLeft;
    brightLabel.attributedText = [self setHSLText:kJL_TXT("light_color_temperature_colorful")];
    
    //L
    if(sW == 320.0){
        sliderSewen_2.frame = CGRectMake(42.5,grayLabel.frame.origin.y+grayLabel.frame.size.height+19, [UIScreen mainScreen].bounds.size.width - 2*47.5, 30);
        if([kJL_GET hasPrefix:@"zh"]){
            darkLabel.frame = CGRectMake(22, grayLabel.frame.origin.y+grayLabel.frame.size.height+23, 25, 25);
            sunshineLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-44, grayLabel.frame.origin.y+grayLabel.frame.size.height+23, 54, 25);
        }else{
            darkLabel.frame = CGRectMake(10, grayLabel.frame.origin.y+grayLabel.frame.size.height+23, 50, 25);
            sunshineLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-50, grayLabel.frame.origin.y+grayLabel.frame.size.height+23, 54, 25);
        }
    }else{
        sliderSewen_2.frame = CGRectMake(42.5,grayLabel.frame.origin.y+grayLabel.frame.size.height+24, [UIScreen mainScreen].bounds.size.width - 2*47.5, 30);
        if([kJL_GET hasPrefix:@"zh"]){
            darkLabel.frame = CGRectMake(22, grayLabel.frame.origin.y+grayLabel.frame.size.height+28, 25, 25);
            sunshineLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-44, grayLabel.frame.origin.y+grayLabel.frame.size.height+28, 54, 25);
        }else{
            darkLabel.frame = CGRectMake(10, grayLabel.frame.origin.y+grayLabel.frame.size.height+28, 50, 25);
            sunshineLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-50, grayLabel.frame.origin.y+grayLabel.frame.size.height+28, 54, 25);
        }
    }
    [sliderSewen_2 setMinimumTrackTintColor:kColor_0000];
    
    darkLabel.contentMode = UIViewContentModeRight;
    darkLabel.attributedText = [self setHSLText:kJL_TXT("light_color_temperature_dark")];
    sunshineLabel.contentMode = UIViewContentModeLeft;
    sunshineLabel.attributedText = [self setHSLText:kJL_TXT("light_color_temperature_sun")];
    ...
    dataArray = @[kJL_TXT("light_model_1"),kJL_TXT("light_model_2"),kJL_TXT("light_model_3"),kJL_TXT("light_model_4"),
                  kJL_TXT("light_model_5"),kJL_TXT("light_model_6"),kJL_TXT("light_model_7"),kJL_TXT("light_model_8")];
    ...
    float h = [UIScreen mainScreen].bounds.size.height - topViewH.constant;
    ...
    [collections registerNib:nib forCellWithReuseIdentifier:@"FunctionCell"];
    ...
    sceneDataArray = @[kJL_TXT("light_scene_mode_1"),kJL_TXT("light_scene_mode_2"),kJL_TXT("light_scene_mode_3"),kJL_TXT("light_scene_mode_4"),
                       kJL_TXT("light_scene_mode_5"),kJL_TXT("light_scene_mode_6"),kJL_TXT("light_scene_mode_7"),kJL_TXT("light_scene_mode_8")
                       ,kJL_TXT("light_scene_mode_9"),kJL_TXT("light_scene_mode_10"),kJL_TXT("light_scene_mode_11"),kJL_TXT("light_scene_mode_12")];
...
- (IBAction)onSwitchChange:(UISwitch *)sender {
    kJLLog(JLLOG_DEBUG,@"--->总开关:%d",sender.isOn);
...
-(void)addColorAnimation{
    ...
    CGPathAddQuadCurveToPoint(path, NULL, sW/2.0, -100, firstView.frame.origin.x+15, firstView.frame.origin.y+15);
...
- (void)colorPickerDidChangeSelection:(RSColorPickerView *)colorPicker
{
    ...
    kJLLog(JLLOG_DEBUG,@"CHANGE ==> R:%d G:%d B:%d",led_r,led_g,led_b);
}
          ...
          withEvent:(UIEvent *)event
{
    ...
    uint8_t led_g = (uint8_t)(cls[1]*255.0);
    uint8_t led_b = (uint8_t)(cls[2]*255.0);
    kJLLog(JLLOG_DEBUG,@"END ==> R:%d G:%d B:%d",led_r,led_g,led_b);
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.separatorInset = UIEdgeInsetsMake(0, 27, 0, 0);
    cell.layoutMargins = UIEdgeInsetsMake(0, 27, 0, 0);
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell.mImageView setImage:[UIImage imageNamed:sceneImageArray[indexPath.row]]];
    [cell.mLabel setText:sceneDataArray[indexPath.row]];
    [cell.mLabel setFont:[UIFont fontWithName:@"PingFang SC" size: 15]];
    [cell.mLabel setTextColor:kDF_RGBA(36, 36, 36, 1.0)];
    return cell;
}
...
-(void)setFlashBtns{
    ...
    bottomView.frame = CGRectMake(0,subTableView.frame.origin.y+subTableView.frame.size.height+10.5,[UIScreen mainScreen].bounds.size.width,100.5);
    ...
    flashFrelabel.attributedText = [self setFlashText:kJL_TXT("light_flash_freq")];
    
    UIView *buttonsView;
    int distance = 0;
    if(sW <=375.0){
        if(sW == 320.0){
            buttonsView = [[UIView alloc] initWithFrame:CGRectMake(6, 48.5, [UIScreen mainScreen].bounds.size.width-2*6, 32)];
            distance = 19;
        }else{
            buttonsView = [[UIView alloc] initWithFrame:CGRectMake(13, 48.5, [UIScreen mainScreen].bounds.size.width-2*13, 32)];
            distance = 33;
        }
    }else{
        buttonsView = [[UIView alloc] initWithFrame:CGRectMake(26, 48.5,[UIScreen mainScreen].bounds.size.width-2*26, 32)];
        distance = 37;
    }    
    ...
    for (int i = 0; i<4; i++) {
        ...
        if(i == 0){
            ...
            [btnFlash_0 setTitle:kJL_TXT("light_flash_quick") forState:UIControlStateNormal];
        ...
        if(i == 1){
            ...
            [btnFlash_1 setTitle:kJL_TXT("light_flash_slow") forState:UIControlStateNormal];
        ...
        if(i == 2){
            ...
            [btnFlash_2 setTitle:kJL_TXT("light_flash_slower") forState:UIControlStateNormal];
        ...
        if(i == 3){
            ...
            [btnFlash_3 setTitle:kJL_TXT("light_flash_music") forState:UIControlStateNormal];
...
-(void)updateUIFromFirework{
    ...
    if(switchAll.on == 0){
        //[DFUITools showText:kJL_TXT("light_tip_open_switch") onView:self.view delay:1.0];
    }
    [self handleSwitch];
