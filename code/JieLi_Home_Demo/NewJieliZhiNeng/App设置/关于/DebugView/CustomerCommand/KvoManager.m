...
-(void)notificationAction:(NSNotification *)note{
    ...
    if([note.name isEqualToString:@"EQ_MODE_CHANGE"]){
        ...
        kJLLog(JLLOG_DEBUG,@"dev:%d,dev.eqArray:%@",dev.eqMode,dev.eqArray);
    }
...
-(void)addEqMode:(NSNotification *)note{
    kJLLog(JLLOG_DEBUG,@"addNote:%@",note.object);
}
...
//        kJLLog(JLLOG_DEBUG,@"%@",change[NSKeyValueChangeNewKey]);
//    }
//    if([keyPath isEqualToString:@"eqMode"]){
//        kJLLog(JLLOG_DEBUG,@"%@",change);
//    }
//    if([keyPath isEqualToString:@"mAncModeCurrent"]){
//        kJLLog(JLLOG_DEBUG,@"%@",change);
//    }
    JLModel_Device *device = object;
    kJLLog(JLLOG_DEBUG,@"device.eqModel:%d",device.eqMode);
    kJLLog(JLLOG_DEBUG,@"device.eqArray:%@",device.eqArray);
}
