...
//PiLink隐私政策的URL
#define PILINK_PRIVACY_POLICY_URL      @"http://cam.jieliapp.com:28111/app/pilink_app_privacy_policy.html"
...
-(void)initUI{
    ...
    titleName.text = kJL_TXT("privacy_policy_name");
    ...
    //       NSMutableAttributedString *titleNameStr = [[NSMutableAttributedString alloc] initWithString:kJL_TXT("privacy_policy_name") attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 18],NSForegroundColorAttributeName: [UIColor colorWithRed:36/255.0 green:36/255.0 blue:36/255.0 alpha:1.0]}];
    ...
    noneLab_1.text = kJL_TXT("network_exception");
...
-(void)actionNetStatus:(AFNetworkReachabilityStatus)status{
    ...
    if (status == AFNetworkReachabilityStatusReachableViaWWAN) {
        ...
        if(kJL_UI_SERIES == 0){ //杰理之家
            privateStr = JIELI_HOME_PRIVACY_POLICY_URL;
        }
        if(kJL_UI_SERIES == 1){ //PiLink
            privateStr = PILINK_PRIVACY_POLICY_URL;
        }
