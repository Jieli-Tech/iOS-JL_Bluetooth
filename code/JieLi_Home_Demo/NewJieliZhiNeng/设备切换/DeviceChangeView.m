...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (type == JLUuidTypeDisconnected) {
        ...
        kJLLog(JLLOG_DEBUG,@"---> 下拉顶栏，连接设备：%@",model.name);
...
-(UIImage *)imageType:(JL_DeviceType)type protocol:(int)ptl{
    switch (type) {
        case JL_DeviceTypeSoundBox:
            return [UIImage imageNamed:@"Theme.bundle/img_soundbox"];
            break;
        case JL_DeviceTypeChargingBin:{
            return [UIImage imageNamed:@"Theme.bundle/chargingBin_small"];
        }break;
    ...
    return  [UIImage imageNamed:@"img_soundbox"];
}
