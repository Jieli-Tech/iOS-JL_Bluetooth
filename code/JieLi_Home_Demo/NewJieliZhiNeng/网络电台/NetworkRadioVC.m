...
- (void)viewDidLoad {
    ...
    kJLLog(JLLOG_DEBUG,@"update Player Status--->1");
...
-(void)requestHttpData{
    ...
    if (netPlayer.provinceIDArray.count > 0) {
        ...
        [DFAction mainTask:^{
            ...
            kJLLog(JLLOG_DEBUG,@"update Player Status--->00");
            [self updatePlayerStatus];
            ...
            if (netPlayer.provinceIDArray.count > 0) {
                ...
                [self->locationManager requestLocationWithReGeocode:YES
                                                    completionBlock:^(CLLocation *location,
                                                                      ...
                                                                      NSError *error) {
                    if (!error) {
                        kJLLog(JLLOG_DEBUG,@"---> Province: %@",regeocode.province);
                    ...
                    [[User_Http shareInstance] requestProvincialRadioList:pv_id Result:^(NSArray * _Nonnull info) {
                        ...
                        if (info.count > 0) {
                            [DFAction mainTask:^{
                                ...
                                /*--- 更新本地电台列表 ---*/
                                [[NetworkPlayer sharedMe] setPlayList:netPlayer.localRadioArray];
                                kJLLog(JLLOG_DEBUG,@"update Player Status--->0");
                                [self updatePlayerStatus];
...
-(void)readCollectionRadio{
    ...
    if (radioJsonPath) {
        ...
        kJLLog(JLLOG_DEBUG,@"Read Collection Radios ---> %lu",(unsigned long)netPlayer.collectionRadioArray.count);
    }else{
        [JL_Tools findPath:NSLibraryDirectory MiddlePath:@"" File:@"RADIO_COLLECT_JSON.txt"];
        kJLLog(JLLOG_DEBUG,@"Create Collection Radios.");
    }
...
-(void)saveCollectionRadio{
    if (netPlayer.collectionRadioArray.count>0) {
        ...
        kJLLog(JLLOG_DEBUG,@"Save Collection Radios ---> %lu",(unsigned long)netPlayer.collectionRadioArray.count);
...
-(void)onBtnPlaceAction:(UIButton*)btn{
    ...
    kJLLog(JLLOG_DEBUG,@"Place ----> %@",placeInfo[@"name"]);
...
-(void)noteNetworkPlayerStatus:(NSNotification*)note{
    kJLLog(JLLOG_DEBUG,@"update Player Status--->2");
    [self updatePlayerStatus];
}
