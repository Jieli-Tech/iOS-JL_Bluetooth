...
-(void)setToBlackList:(JL_EntityM *)entity{
    //kJLLog(JLLOG_DEBUG,@"--->Set Blacklist Name:%@ Seq:%d",entity.mItem,entity.mSeq);
...
-(void)removeToBlackList:(JL_EntityM *)entity{
    ...
    //kJLLog(JLLOG_DEBUG,@"----> Rest Black List count:%lu",(unsigned long)blackList.count);
}
...
-(BOOL)isInBlackList:(JL_EntityM *)entity{
    ...
    for (BlackMgr *item in blackList) {
        if ([item.uuid isEqualToString:entity.mPeripheral.identifier.UUIDString]) {
            ...
            //kJLLog(JLLOG_DEBUG,@"------>Name:%@  SEQ:%d  Black:%d",entity.mItem,entity.mSeq,item.seq);
            if (entity.mSeq > item.seq) {
                if (entity.mSeq - item.seq >= 5) {
                    /*--- 同步SEQ ---*/
                    kJLLog(JLLOG_DEBUG,@"【%@】--->0 同步SEQ：%d ",entity.mItem,entity.mSeq);
                ...
                if (entity.mSeq <= 5 && item.seq > 5) {
                    /*--- 同步SEQ ---*/
                    kJLLog(JLLOG_DEBUG,@"【%@】--->1 同步SEQ：%d ",entity.mItem,entity.mSeq);
                    [self setToBlackList:entity];
                }
                ret = YES;
            }
    ...
    //kJLLog(JLLOG_DEBUG,@"------>Name:%@  Bound:%d",entity.mItem,ret);
    return ret;
}
