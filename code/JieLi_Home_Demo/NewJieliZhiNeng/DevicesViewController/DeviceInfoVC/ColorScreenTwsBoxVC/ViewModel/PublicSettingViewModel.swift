    ...
    private func parepareScreenSaver(_ chain: JLTaskChain) {
        ...
        chain.addTask {  _, completion in
            if self.chipType == .AC701N {
                ...
                self.dialUnitMgr?.getFileList(.FLASH, count: 100) { _, _ in
                    self.dialUnitMgr?.dialGetCurrent { currentDialMode in
                        if let mode = currentDialMode {
                            self.dialUnitMgr?.dialGetDial("/" + mode.fileName) { currentScreenSaverMode, err in
        ...
        chain.addTask { [weak self] _, completion in
            self?.dialUnitMgr?.getFileList(.FLASH, count: 100, completion: { list, _ in
                if let list = list {
                    self?.fileModelList.accept(list)
                    for model in list {
                        JLLogManager.logLevel(.DEBUG, content: "==>model.name: \(model.fileName),crc: \(model.crc),cluster: \(model.fileClus),index: \(model.fileIndex),size: \(model.size)")
                    }
                }
                completion(nil, nil)
            })
