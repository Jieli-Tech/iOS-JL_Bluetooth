...
@interface DeviceInfoVC ()<HeadSetControlDelegate,NormalSettingDelegate,ReNameViewDelegate,MofifyNameAlertDelegate,HeadsetDenoisePtl>{
    ...
    NormalSettingView *auracastReceiverView;
    NormalSettingView *auracastLancerView;
...
-(void)localDeviceJson{
    ...
    switch (devModel.sdkType) {
        case JL_SDKTypeAI:
        case JL_SDKTypeDongle:
        case JL_SDKTypeCommon:
...
-(void)initWithData{
    ...
    JL_SDKType sdkType = [[[JL_RunSDK sharedMe] mBleEntityM].mCmdManager getDeviceModel].sdkType;
    ...
    if (bleType == JL_DeviceTypeTWS
        || sdkType == JL_SDKType697xTWS
        || sdkType == JL_SDKTypeManifestEarphone) {
...
-(void)initWithUI{
    ...
    if ([[AppStatusManager shareInstance] isSupportAuracast]) {
        [[DeviceInfoViewModel shareInstance] onStart];
        if ([AppStatusManager shareInstance].isAuracastReceiver) {
            if(auracastReceiverView){
                auracastReceiverView.frame = CGRectMake(0, eHeight + interval, width, 60);
                interval+=(60+addHight);
            }else{
                auracastReceiverView = [[NormalSettingView alloc] initWithFrame:CGRectMake(0, eHeight + interval, width, rowHight)];
                auracastReceiverView.delegate = self;
                auracastReceiverView.tag = 5;
                NSMutableArray *array = [NSMutableArray new];
                NormalSettingObject *obj = [NormalSettingObject new];
                obj.img = [UIImage imageNamed:@"function_icon_auracast"];
                obj.funcStr = kJL_TXT("Listen Auracast Broadcast");
                [array addObject:obj];
                
                [JLUI_Effect addShadowOnView:auracastReceiverView];
                [auracastReceiverView config:array];
                auracastReceiverView.layer.masksToBounds = YES;
                [scrollView addSubview:auracastReceiverView];
                interval+=(rowHight+addHight);
            }
        }
        
        if ([[AppStatusManager shareInstance] isAuracastLancer]) {
            if(auracastLancerView){
                auracastLancerView.frame = CGRectMake(0, eHeight + interval, width, 60);
                interval+=(60+addHight);
            }else{
                auracastLancerView = [[NormalSettingView alloc] initWithFrame:CGRectMake(0, eHeight + interval, width, rowHight)];
                auracastLancerView.delegate = self;
                auracastLancerView.tag = 6;
                NSMutableArray *array = [NSMutableArray new];
                NormalSettingObject *obj = [NormalSettingObject new];
                obj.img = [UIImage imageNamed:@"function_icon_auracast_settle"];
                obj.funcStr = kJL_TXT("Configure Auracast broadcast");
                [array addObject:obj];
                
                [JLUI_Effect addShadowOnView:auracastLancerView];
                [auracastLancerView config:array];
                auracastLancerView.layer.masksToBounds = YES;
                [scrollView addSubview:auracastLancerView];
                interval+=(rowHight+addHight);
            }
        }
    }
    //MARK: - 双设备连接UI
    if (bleSDK.mBleEntityM.mCmdManager.mTwsManager.supports.isSupportDragWithMore){
        
        NSMutableArray *multiArray = [NSMutableArray new];
        NormalSettingObject *obj = [NormalSettingObject new];
        obj.img = [UIImage imageNamed:@"function_icon_connection_cell"];
    ...
    if (self.headsetDict[@"KEY_SETTING"] && isKeySetting == YES) {
        ...
        if (headSetView) {
            ...
            if (bleType == JL_DeviceTypeTWS
                || deviceType == JL_SDKTypeManifestEarphone
                || deviceType == JL_SDKType697xTWS) {
...
-(void)noremalSetting:(NormalSettingView *)view Selected:(NormalSettingObject *)object{
    ...
    if ([view isEqual:auracastReceiverView]) {
        AuracastViewController *vc = [[AuracastViewController alloc] init];
        [self.navigationController pushViewController:vc animated:true];
    }
    if ([view isEqual:auracastLancerView]) {
        LancerViewController *vc = [[LancerViewController alloc] init];
        [self.navigationController pushViewController:vc animated:true];
    }
