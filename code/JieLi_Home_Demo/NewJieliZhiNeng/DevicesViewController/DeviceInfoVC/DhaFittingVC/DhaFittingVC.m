...
-(void)steptUI{
    ...
    self.naviView.titleLab.text = kJL_TXT("hearing_aid_fitting");
...
-(void)makeLayout{
    [headCell mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.naviView.mas_bottom).offset(16);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([item.type isEqualToString: DoubleFitter]) {
        ...
        cell.typeLab.font = FontMedium(12);
...
- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath{
    kJLLog(JLLOG_DEBUG,@"cell-->%s:%d",__func__,canbeEdit);
    return canbeEdit;
}
