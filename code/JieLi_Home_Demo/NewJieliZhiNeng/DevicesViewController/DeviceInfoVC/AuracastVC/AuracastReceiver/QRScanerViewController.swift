...
class QRScanerViewController: BaseViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    override func initUI() {
        navigationView.title = R.localStr.scanQRCode()
        navigationView.titleLab.textColor = .white
        navigationView.leftBtn.setImage(R.image.icon_return_white(), for: .normal)
        view.addSubview(albumBtn)
        view.addSubview(flashLightBtn)
        view.backgroundColor = .clear
        navigationView.bgView.backgroundColor = .clear
    ...
    override func initData() {
            ...
            PHPhotoLibrary.requestAuthorization { status in
                DispatchQueue.main.async {
                    if status == .authorized {
                        ...
                        JLLogManager.logLevel(.ERROR, content: "user reject photo library")
        ...
        flashLightBtn.rx.tap.subscribe(onNext: { [weak self] in
            ...
            do {
                ...
                JLLogManager.logLevel(.ERROR, content: error.localizedDescription)
            }
        }).disposed(by: disposeBag)
    }
    ...
    private func checkCameraPermissionStatus() {
        ...
        DispatchQueue.main.async {
            ...
            case .authorized:
                // 摄像头权限已经被授权
                JLLogManager.logLevel(.DEBUG, content: "Camera permission is granted.")
                self.setupCaptureSession()
            case .denied:
                // 摄像头权限已被拒绝
                JLLogManager.logLevel(.DEBUG, content: "Camera permission was denied.")
                // 你可以提示用户前往设置页面开启权限
                self.showSettingsAlert()
            case .restricted:
                // 摄像头权限受到限制（例如在家长控制模式下）
                JLLogManager.logLevel(.DEBUG, content: "Camera access is restricted.")
                self.showSettingsAlert()
            case .notDetermined:
                // 摄像头权限尚未请求
                JLLogManager.logLevel(.DEBUG, content: "Camera permission not yet determined.")
                AVCaptureDevice.requestAccess(for: .video) { granted in
                    DispatchQueue.main.async {
                        if granted {
                            JLLogManager.logLevel(.DEBUG, content: "Camera permission granted.")
                            self.setupCaptureSession()
                        } else {
                            JLLogManager.logLevel(.DEBUG, content: "Camera permission not granted.")
                            // 摄像头权限请求被拒绝
                            self.showSettingsAlert()
                        }
                    }
            ...
            @unknown default:
                // 处理未知的情况
                JLLogManager.logLevel(.DEBUG, content: "Unknown camera permission status.")
                self.showSettingsAlert()
            }
        }
    }
    ...
    func metadataOutput(_: AVCaptureMetadataOutput,
        ...
        if let metadataObject = metadataObjects.first {
            ...
            JLLogManager.logLevel(.DEBUG, content: "Scanned QR Code: \(stringValue)")
    ...
    func imagePickerController(_: UIImagePickerController,
        ...
        if let image = info[.originalImage] as? UIImage {
            ...
            guard let features = detector?.features(in: ciImage) as? [CIQRCodeFeature], features.count > 0 else {
                JLLogManager.logLevel(.WARN, content: "No QR codes found.")
                AlertManager.windows()?.makeToast(R.localStr.failedToLoadQRCode(), position: .center)
                return
            }
            for feature in features {
                if let qrCodeData = feature.messageString {
                    JLLogManager.logLevel(.DEBUG, content: "QR Code Data: \(qrCodeData)")
