...
-(void)stepUI{
    ...
    activeView.activityIndicatorViewStyle =  UIActivityIndicatorViewStyleMedium;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    }else if(entity.mType == JL_DeviceTypeChargingBin){
        cell.deviceImgv.image = [UIImage imageNamed:@"Theme.bundle/location_icon_chargingBin_black"];
    }else{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSNumber *vidNumber = [NSNumber numberWithLong:strtoul(bleEntity.mUID.UTF8String, 0, 16)];
    ...
    [bleEntity.mCmdManager cmdRequestDeviceImageVid:vidStr Pid:pidStr
        ...
        /*--- 连接设备 ---*/
        kJLLog(JLLOG_DEBUG,@"---> 搜索界面，连接设备：%@，Edr：%@",bleEntity.mItem,bleEntity.mEdr);
        
        __strong typeof(wSelf) sSelf = wSelf;
        sSelf->currentConnectIndex = 0;
        [self connectWithEntity:bleEntity];
    }];
}
        ...
        if (status == JL_EntityM_StatusPaired) {
            ...
            if(status == JL_EntityM_StatusConnectTimeout ||
               status == JL_EntityM_StatusConnectFail) {
                ...
                kJLLog(JLLOG_DEBUG, @"search view 重新连接，第%d次",sSelf->currentConnectIndex);
