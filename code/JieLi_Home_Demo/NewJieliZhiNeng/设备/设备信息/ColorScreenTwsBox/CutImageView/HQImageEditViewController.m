...
static inline UIEdgeInsets hq_safeAreaInset(void) {
...
//    self.hq_navigationBarHide = YES;
    ...
    [self.view addSubview:self.existBtn];
...
- (void)viewDidLayoutSubviews {
    ...
    [self.existBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.view.mas_safeAreaLayoutGuideTop).offset(46);
        make.width.equalTo(@40);
    }];
    
    [self.rotateButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(@10);
        make.bottom.equalTo(self.actionView.mas_top).offset(-10);
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [_actionView setUserInteractionEnabled:true];
}
...
//        UIImage *originImage = [self.captureView captureOriginalImage];
        UIImage *originImage = [self rotateImageToUp:self.originImage];
...
- (UIImage *)rotateImageToUp:(UIImage *)image {
...
-(void)existBtnAction:(UIButton *)sender {
    if ([self.delegate respondsToSelector:@selector(editControllerDidClickCancel:)]) {
        [self.delegate editControllerDidClickCancel:self];
    }
}
...
- (void)buttonAction:(UIButton *)button {
    ...
    [self action:self.actionView didClickButton:button atIndex:button.tag];
...
- (HQEditImageActionView *)actionView {
    if (!_actionView) {
        ...
        [_actionView setUserInteractionEnabled:false];
    }
    return _actionView;
}
...
- (HQEditImageEditView *)editView {
    if (!_editView) {
        _editView = [[HQEditImageEditView alloc] initWithMargin:UIEdgeInsetsMake(self.topSpace + hq_safeAreaInset().top, self.leftSpace, self.bottomSpace, self.rightSpace) size:CGSizeMake(self.editViewSize.width, self.editViewSize.height) WithModel:self.model];
...
-(UIButton *)existBtn{
    if (!_existBtn) {
        _existBtn = [UIButton new];
        [_existBtn setImage:[UIImage imageNamed:@"Theme.bundle/local_icon_return.png"] forState:UIControlStateNormal];
        [_existBtn addTarget:self action:@selector(existBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _existBtn;
}
...
- (UIButton *)rotateButton {
    if (!_rotateButton) {
        ...
        [_rotateButton setImage:[UIImage imageNamed:@"Theme.bundle/icon_revolve_nor"] forState:UIControlStateNormal];
        _rotateButton.tag = 0;
