...
@property (nonatomic, assign) JLDialInfoExtentedModel *dialInfoModel;
@end
...
- (UIView *)hqmaskView {
    if (!_hqMaskView) {
        ...
        _hqMaskView.backgroundColor = [UIColor colorWithWhite:0 alpha:.65f];
        CGFloat radius = 0;
        
        if (_dialInfoModel.shape == 0x01){//圆
            radius = self.previewSize.width/2;
        }else if (_dialInfoModel.shape == 0x02){//方
            radius = 0;
        }else if (_dialInfoModel.shape == 0x03){//圆角矩形
            radius = _dialInfoModel.radius/2;
        }
        
        UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, 0, self.previewSize.width + self.margin.left + self.margin.right, self.previewSize.height + self.margin.top + self.margin.bottom) cornerRadius:0];
        UIBezierPath *cropPath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(self.margin.left, self.margin.top, self.previewSize.width, self.previewSize.height) cornerRadius:radius];
        ...
        borderLayer.path = cropPath.CGPath;
        borderLayer.fillColor = [UIColor colorWithWhite:0 alpha:0].CGColor; // 修改此处，不再设置为clearColor
        borderLayer.strokeColor = [UIColor whiteColor].CGColor;
        borderLayer.lineWidth = 2;
        [_hqMaskView.layer addSublayer:borderLayer];
...
-(UIView *)cropWrap{
    if (!_cropWrap) {
        _cropWrap = [[UIView alloc] initWithFrame:CGRectZero];
        _cropWrap.layer.mask.cornerRadius = 12;
        _cropWrap.layer.masksToBounds = true;
        _cropWrap.layer.borderColor = UIColor.whiteColor.CGColor;
        _cropWrap.layer.borderWidth = 2;
    }
    return _cropWrap;
}
@end
