...
- (void)setFuncDict:(NSDictionary *)funcDict{
    ...
    for (NSDictionary *item in arr0) {
        ...
        if (value == nil) {
            value = @"unKnow";
            kJLLog(JLLOG_DEBUG,@"unKnow value funcKey:%@",key);
        }
...
-(NSString *)keySetting:(int)num withKey:(NSString *)func_key{
    ...
    for (NSDictionary *item in arr0) {
        if ([item[@"value"] intValue] == num) {
            ...
            while ((key = [enumerator nextObject])) {
                ...
                if (value == nil) {
                    value = @"unKnow";
                    kJLLog(JLLOG_DEBUG,@"unKnow value：num %d, funcKey:%@",num,func_key);
                }
                return value;
            }
            break;
    ...
    kJLLog(JLLOG_DEBUG,@"unKnow value2：num %d, funcKey:%@",num,func_key);
