...
-(void)updateWithOTAResult:(JL_OTAUrlResult)result
    ...
    kJLLog(JLLOG_DEBUG,@"---> 服务器 Version:%@",version);
    
    JLModel_Device *model = [self.otaEntity.mCmdManager outputDeviceModel];
    NSString *currentFireCode = model.versionFirmware;
    kJLLog(JLLOG_DEBUG,@"---> 固件 Version:%@",currentFireCode);
    ...
    if (type == JLDeviceChangeTypeSomethingConnected) {
        ...
        if (otaUuid.length >0) {
            ...
            if (uuidType == JLUuidTypeNeedOTA) {
                //重连后继续升级
                kJLLog(JLLOG_DEBUG,@"---> OTA成功回连设备:%@",self.otaEntity.mItem);
                [self upgradeAction];
                return;
            }
            ...
            if (self.otaBleAddr) {
                //重连后继续升级
                kJLLog(JLLOG_DEBUG,@"---> OTA成功回连设备2:%@",self.otaEntity.mItem);
                [self upgradeAction];
                self.otaBleAddr = nil;
                return;
            }
...
-(void)noteEnterBackground:(NSNotification*)note{
    ...
    if (isUpdateing) {
        ...
        [JL_Tools delay:1.0 Task:^{
            ...
            kJLLog(JLLOG_DEBUG,@"---> OTA 用户异常操作，断开BLE.");
