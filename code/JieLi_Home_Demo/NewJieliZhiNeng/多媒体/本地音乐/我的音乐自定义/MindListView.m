...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.numberLab.font = FontMedium(16);
...
-(void)playMusic:(NSIndexPath *)indexPath{
    ...
    kJLLog(JLLOG_DEBUG,@"UI关闭ID3信息。33");
    ...
    [DFAction delay:0.15 Task:^{
        ...
        if (entity.mType != JL_DeviceTypeTWS ) {
            kJLLog(JLLOG_DEBUG,@"--->设备音量：%.1f",val);
            player.mPlayer.volume = val;
        }else{
            [DFAudioPlayer getPhoneVolume];
            kJLLog(JLLOG_DEBUG,@"--->手机音量：%.1f",val);
...
-(void)scrollToNowMusic{
    ...
    if ([_listTable numberOfRowsInSection:0] > 0) {
