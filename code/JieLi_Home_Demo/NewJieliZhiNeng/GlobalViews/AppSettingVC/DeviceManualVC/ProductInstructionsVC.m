...
-(void)initUI{
    ...
    titleName.text = kJL_TXT("device_instructions");
...
-(void)downloadInfoImage{
    ...
    if(bleSDK.mBleEntityM){
        ...
        [self startLoadingView:kJL_TXT("loading") Delay:10];
...
-(UIScrollView *)imgScroll {
    if (!imgScroll) {
        imgScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, kJL_HeightNavBar, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height-kJL_HeightNavBar)];
...
-(UIImageView *)myImageView {
    if (!myImageView) {
        CGFloat imgH = [self imgContentHeight];
        if(imgH>0){
            myImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, imgH)];
...
-(CGFloat)imgContentHeight{
    ...
    CGFloat imgH = imgHeight * ([UIScreen mainScreen].bounds.size.width / imgWidth);
    return imgH;
}
...
-(void)mProductNoUpload{
    ...
    noneImv.hidden = NO;
    noneLab_1.hidden = NO;
    noneLab_1.text = kJL_TXT("No data yet, please connect the device first");
    noneLab_2.hidden = YES;
}
