...
@objcMembers class MediasFuncsView: SwBasicView, JLDevPlayerCtrlDelegate, LanguagePtl, JLConfigPtl {
    ...
    override func initUI() {
        super.initUI()
        backgroundColor = UIColor(fromHexString: "#F8FAFE")
        ...
        itemsArray.bind(to: collectView.rx.items(cellIdentifier: "MediasFuncsCell", cellType: MediasFuncsCell.self)) { _, model, cell in
    ...
    func updateWith(model: JLModel_Device) {
        ...
        if JL_RunSDK.sharedMe().twsConfigMode?.isSupportTranslate == true {
            items.append(allItems[14])
            items.append(allItems[15])
        }
        
        if JL_RunSDK.sharedMe().twsConfigMode?.isSupportHealthData == true {
            items.append(allItems[16])
        }
    ...
    private func addAll() {
        ...
        let nameArr: [String] = [
            "multi_media_local",
            "multi_media_usb_drive",
            "multi_media_sd_card",
            "multi_media_fm_launch",
            "multi_media_fm_receive",
            "multi_media_line_in",
            "multi_media_light_settings",
            "default_alarm_name",
            "multi_media_search_device",
            "multi_media_net_radio",
            "multi_media_sound_card",
            "multi_media_watch",
            "SPDIF",
            "PC",
            "Translation",
        ...
        let imgArr: [String] = [
            "Theme.bundle/mul_icon_local_nor",
            ...
            allItems
                .append(
                    MediasModel(
                        ...
                        isSelect: false,
                        type: MediasFuncType(rawValue: UInt8(i))!
                    )
                )
        }
        itemsArray
    ...
    private func cardStatusSel() {
        guard let model = JL_RunSDK.sharedMe().mBleEntityM?.mCmdManager.getDeviceModel() else {
            return
        }
...
extension MediasFuncsView {
    ...
    func jlDevPlayerCtrl(
        _: JLDevPlayerCtrl,
        fileName _: String,
        currentClus _: UInt32
    ) {}
    // MARK: - handle tws config
...
class MediasFuncsCell: UICollectionViewCell {
    ...
    func configModel(
        ...
        if model.isSelect {
            centerView.layer.shadowColor = UIColor(
                fromRGBAArray: [
                    0,
                    0,
                    0,
            ...
            centerView.layer.shadowOffset = CGSize(
                width: 0,
                height: 0
            )
