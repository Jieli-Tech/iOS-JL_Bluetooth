    ...
    if (self.rtcmodel.ringInfo) {
        ...
        if(device.rtcAlarmType == JL_RTCAlarmTypeYES){
            if(mAlarmSetting.isTime==1 && mAlarmSetting.isInterval==1 && mAlarmSetting.isCount==1){ //响铃时长、再响间隔时间、再响响铃次数
                //响铃时长
                NSString *timeStr =  [NSString stringWithFormat:@"%d%@",mAlarmSetting.time,kJL_TXT("min2_format")];
                //再响间隔(间隔时间+响铃次数)
                NSString *zaiXiangStr =  [NSString stringWithFormat:@"%d%@%s%d%@",mAlarmSetting.interval,kJL_TXT("min2_format"),",",mAlarmSetting.count,kJL_TXT("count_format_1")];
            ...
            if(mAlarmSetting.isTime == 1 && mAlarmSetting.isInterval ==1 && mAlarmSetting.isCount == 0){ //响铃时长、再响间隔时间
                ...
                NSString *timeStr =  [NSString stringWithFormat:@"%d%@",mAlarmSetting.time,kJL_TXT("min2_format")];
                //再响间隔(间隔时间+再响间隔时间)
                NSString *zaiXiangStr =  [NSString stringWithFormat:@"%d%@",mAlarmSetting.interval,kJL_TXT("min2_format")];
                nameArray = @[self.rtcmodel.rtcName,ringName,timeStr,zaiXiangStr];
            }
            if(mAlarmSetting.isTime==1 && mAlarmSetting.isCount==1 && mAlarmSetting.isInterval ==0){ //响铃时长、再响响铃次数
                ...
                NSString *timeStr =  [NSString stringWithFormat:@"%d%@",mAlarmSetting.time,kJL_TXT("min2_format")];
                //再响间隔(响铃次数)
                NSString *zaiXiangStr =  [NSString stringWithFormat:@"%d%@",mAlarmSetting.count,kJL_TXT("count_format_1")];
                nameArray = @[self.rtcmodel.rtcName,ringName,timeStr,zaiXiangStr];
            }
...
- (IBAction)commitBtnAction:(id)sender {
    ...
    [entity.mCmdManager.mAlarmClockManager cmdRtcSetArray:@[self.rtcmodel] Result:^(JL_CMDStatus status, uint8_t sn, NSData * _Nullable data) {
...
- (void)deleteBtnAction:(UIButton *)btn {
    JL_EntityM *entity = [[JL_RunSDK sharedMe] mBleEntityM];
    [entity.mCmdManager.mAlarmClockManager cmdRtcDeleteIndexArray:@[@(self.rtcmodel.rtcIndex)] Result:^(JL_CMDStatus status, uint8_t sn, NSData * _Nullable data) {
...
-(void)initWithData{
    ...
    if(device.rtcAlarmType == JL_RTCAlarmTypeYES){
        if(mAlarmSetting.isTime==1 && mAlarmSetting.isInterval==1 && mAlarmSetting.isCount==1){ //响铃时长、再响间隔时间、再响响铃次数
            itemArray = @[kJL_TXT("named"),kJL_TXT("bell"),kJL_TXT("alarm_bell_time"),kJL_TXT("alarm_bell_interval")];
        }
        ...
        if(mAlarmSetting.isTime==0 &&mAlarmSetting.isInterval==1 && mAlarmSetting.isCount==1){
            itemArray = @[kJL_TXT("named"),kJL_TXT("bell"),kJL_TXT("alarm_bell_interval")];
        }
        if(mAlarmSetting.isTime==1 && mAlarmSetting.isInterval ==0 && mAlarmSetting.isCount ==0){
            itemArray = @[kJL_TXT("named"),kJL_TXT("bell"),kJL_TXT("alarm_bell_time")];
        }
        if(mAlarmSetting.isTime==0 && mAlarmSetting.isInterval ==1 && mAlarmSetting.isCount ==0){
            itemArray = @[kJL_TXT("named"),kJL_TXT("bell"),kJL_TXT("alarm_bell_interval")];
        }
        ...
        itemArray = @[kJL_TXT("named"),kJL_TXT("bell")];
    }
    ecRepeatArr = [NSMutableArray new];
    repeatItemArr = @[kJL_TXT("alarm_repeat_mon"),kJL_TXT("alarm_repeat_tue"),kJL_TXT("alarm_repeat_wed"),kJL_TXT("alarm_repeat_thu"),kJL_TXT("alarm_repeat_fri"),kJL_TXT("alarm_repeat_sat"),kJL_TXT("alarm_repeat_sun")];
    if (self.createType == NO) {
        ...
        self.rtcmodel.rtcName = kJL_TXT("default_alarm_name");
        ...
        if (device.rtcDfRings.count>0) {
            ...
            nameArray = @[kJL_TXT("default_alarm_name"),ring.name];
            ...
            nameArray = @[kJL_TXT("default_alarm_name")];
            itemArray = @[kJL_TXT("named")];
        }
...
-(void)initWithUI{
    ...
    [deleteBtn setTitle:kJL_TXT("alarm_delete") forState:UIControlStateNormal];
    ...
    reNameVew = [[ReNameView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
    reNameVew.sizeLength = 10;
    reNameVew.type = 0;
    reNameVew.delegate = self;
    
    if (self.createType) {
        titleLab.text = kJL_TXT("alarm_create_title");
        deleteBtn.hidden = YES;
        scrollView.contentSize = CGSizeMake(sw, tableView.frame.size.height);
    }else{
        titleLab.text = kJL_TXT("alarm_edit_title");
        deleteBtn.hidden = NO;
...
-(void)caculateCyc{
    ...
    for (NSString *item in ecRepeatArr) {
        if([item isEqualToString:kJL_TXT("alarm_repeat_mon")]) [array addObject:@"1"];
        if([item isEqualToString:kJL_TXT("alarm_repeat_tue")]) [array addObject:@"2"];
        if([item isEqualToString:kJL_TXT("alarm_repeat_wed")]) [array addObject:@"3"];
        if([item isEqualToString:kJL_TXT("alarm_repeat_thu")]) [array addObject:@"4"];
        if([item isEqualToString:kJL_TXT("alarm_repeat_fri")]) [array addObject:@"5"];
        if([item isEqualToString:kJL_TXT("alarm_repeat_sat")]) [array addObject:@"6"];
        if([item isEqualToString:kJL_TXT("alarm_repeat_sun")]) [array addObject:@"7"];
    }
...
-(void)sendDataToDevice:(int) index{
    ...
    [entity.mCmdManager.mAlarmClockManager cmdRtcOperate:0x01 Index:index Setting:setting
...
-(void)updaateAlarmUI{
    ...
    NSString *ringName = [[NSString alloc] initWithData:self.rtcmodel.ringInfo.data encoding:NSUTF8StringEncoding];
    //响铃时长
    NSString *timeStr =  [NSString stringWithFormat:@"%d%@",self->mTime,kJL_TXT("min2_format")];
    //再响间隔(间隔时间+响铃次数)
    NSString *zaiXiangStr =  [NSString stringWithFormat:@"%d%@%s%d%@",self->mInterval,kJL_TXT("min2_format"),",",self->mCount,kJL_TXT("count_format_1")];
    if (self.rtcmodel.ringInfo) {
