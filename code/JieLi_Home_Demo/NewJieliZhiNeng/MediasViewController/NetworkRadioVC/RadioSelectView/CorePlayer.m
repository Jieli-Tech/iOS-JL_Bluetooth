...
-(void)playWithUrl:(NSString *)url{
    ...
    kJLLog(JLLOG_DEBUG,@"---> Net Play:%@",url);
...
- (void)loadStateDidChange:(NSNotification*)notification
{
    ...
    if ((loadState & IJKMPMovieLoadStatePlaythroughOK) != 0) {
        kJLLog(JLLOG_DEBUG,@"loadStateDidChange: IJKMPMovieLoadStatePlaythroughOK: %d\n", (int)loadState);
    } else if ((loadState & IJKMPMovieLoadStateStalled) != 0) {
        kJLLog(JLLOG_DEBUG,@"loadStateDidChange: IJKMPMovieLoadStateStalled: %d\n", (int)loadState);
    } else {
        kJLLog(JLLOG_DEBUG,@"loadStateDidChange: ???: %d\n", (int)loadState);
    }
}
...
- (void)moviePlayBackDidFinish:(NSNotification*)notification
{
    ...
    switch (reason)
    {
        case IJKMPMovieFinishReasonPlaybackEnded:
            kJLLog(JLLOG_DEBUG,@"playbackStateDidChange: IJKMPMovieFinishReasonPlaybackEnded: %d\n", reason);
            break;
        case IJKMPMovieFinishReasonUserExited:
            kJLLog(JLLOG_DEBUG,@"playbackStateDidChange: IJKMPMovieFinishReasonUserExited: %d\n", reason);
            break;
        case IJKMPMovieFinishReasonPlaybackError:
            kJLLog(JLLOG_DEBUG,@"playbackStateDidChange: IJKMPMovieFinishReasonPlaybackError: %d\n", reason);
            self.status = DFNetPlayer_STATUS_STOP;
            [self showErrorUI];
            [JL_Tools post:@"kUI_NETPLAY_ERR" Object:self];
            break;
        default:
            kJLLog(JLLOG_DEBUG,@"playbackPlayBackDidFinish: ???: %d\n", reason);
            break;
    }
}
...
{
    kJLLog(JLLOG_DEBUG,@"mediaIsPreparedToPlayDidChange\n");
...
- (void)moviePlayBackStateDidChange:(NSNotification*)notification
{
    ...
    switch (self.player.playbackState)
    {
        case IJKMPMoviePlaybackStateStopped: {
            kJLLog(JLLOG_DEBUG,@"IJKMPMoviePlayBackStateDidChange %d: stoped", (int)self.player.playbackState);
            _status = DFNetPlayer_STATUS_STOP;
            if ([_delegate respondsToSelector:@selector(didChangeStatus:)]) {
        ...
        case IJKMPMoviePlaybackStatePlaying: {
            kJLLog(JLLOG_DEBUG,@"IJKMPMoviePlayBackStateDidChange %d: playing", (int)self.player.playbackState);
            _status = DFNetPlayer_STATUS_PLAY;
            if ([_delegate respondsToSelector:@selector(didChangeStatus:)]) {
        ...
        case IJKMPMoviePlaybackStatePaused: {
            kJLLog(JLLOG_DEBUG,@"IJKMPMoviePlayBackStateDidChange %d: paused", (int)self.player.playbackState);
            _status = DFNetPlayer_STATUS_PAUSE;
            if ([_delegate respondsToSelector:@selector(didChangeStatus:)]) {
        ...
        case IJKMPMoviePlaybackStateInterrupted: {
            kJLLog(JLLOG_DEBUG,@"IJKMPMoviePlayBackStateDidChange %d: interrupted", (int)self.player.playbackState);
            break;
        }
        case IJKMPMoviePlaybackStateSeekingForward:
        case IJKMPMoviePlaybackStateSeekingBackward: {
            kJLLog(JLLOG_DEBUG,@"IJKMPMoviePlayBackStateDidChange %d: seeking", (int)self.player.playbackState);
            break;
        }
        default: {
            kJLLog(JLLOG_DEBUG,@"IJKMPMoviePlayBackStateDidChange %d: unknown", (int)self.player.playbackState);
            break;
        }
    }
}
