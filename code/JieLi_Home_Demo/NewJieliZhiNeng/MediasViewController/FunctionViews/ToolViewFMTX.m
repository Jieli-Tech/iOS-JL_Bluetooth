...
-(void)onFMPickView:(FMPickView *)view didChange:(NSInteger)fmPoint{
    kJLLog(JLLOG_DEBUG,@"FM change: %ld",(long)fmPoint);
...
-(void)onFMPickView:(FMPickView *)view didSelect:(NSInteger)fmPoint{
    ...
    if (model.currentFunc == JL_FunctionCodeFMTX) {
        [entity.mCmdManager.mTwsManager cmdSetFMTX:fmPoint];
    }
}
...
- (IBAction)btn_input_Fm:(id)sender{
    UIAlertController *actionSheet = [UIAlertController alertControllerWithTitle:kJL_TXT("send_fm_point") message:nil
