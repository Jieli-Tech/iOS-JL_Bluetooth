...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.frame = CGRectMake(0, kJL_HeightStatusBar+44, sW, 220);
        ...
        [fmManagerView setFmManagerSelect:^(uint16_t fmSelect) {
            ...
            if (model.currentFunc == JL_FunctionCodeFM) {
                if(self->imageGif.hidden == NO){
                    [DFUITools showText:kJL_TXT("searching") onView:strongSelf delay:1.0];
                    return;
                }
                [entity.mCmdManager.mFmManager cmdFm:JL_FCmdFMFrequencySelect
        ...
        NSURL *gifUrl = [[NSBundle mainBundle] URLForResource:@"search_fm" withExtension:@"gif"];
        [imageGif sd_setImageWithURL:gifUrl];
...
-(void)onFMPickView:(FMPickView *)view didSelect:(NSInteger)fmPoint{
    kJLLog(JLLOG_DEBUG,@"FM Select: %ld",(long)fmPoint);
    fmCurrent = fmPoint;
    JL_EntityM *entity = [[JL_RunSDK sharedMe] mBleEntityM];
    JLModel_Device *model = [entity.mCmdManager outputDeviceModel];
    if (model.currentFunc == JL_FunctionCodeFM) {
...
-(void)updateFmtxUI:(uint16_t)fmPoint{
    if (fmPoint<875) {
        kJLLog(JLLOG_DEBUG,@"---> set FmPoint:%d",fmPoint);
        return;
    }
...
//    }
    if (model.currentFunc == JL_FunctionCodeFM) {
        [entity.mCmdManager.mFmManager cmdFm:JL_FCmdFMSearch
...
- (IBAction)btn_tap_pp:(id)sender {
    ...
    if (model.currentFunc == JL_FunctionCodeFM) {
        ...
        [entity.mCmdManager.mFmManager cmdFm:JL_FCmdFMPP
                   Saerch:0x00 Channel:0x00
                Frequency:0x00 Result:nil];
    }
}
- (IBAction)btn_nextPoint:(id)sender {
    ...
    if (model.currentFunc == JL_FunctionCodeFM) {
        ...
        [entity.mCmdManager.mFmManager cmdFm:JL_FCmdFMChannelNext
                   Saerch:0x00 Channel:0x00
                Frequency:0x00 Result:nil];
    }
}
...
//        kJLLog(JLLOG_DEBUG,@"--->Search View Select Index: %ld",(long)index);
//
//        JL_EntityM *entity = [[JL_RunSDK sharedMe] mBleEntityM];
//        JLModel_Device *model = [entity.mCmdManager outputDeviceModel];
//        if (index == 0) {
...
- (IBAction)btn_input_Fm:(id)sender {
    UIAlertController *actionSheet = [UIAlertController alertControllerWithTitle:kJL_TXT("freq_point") message:nil
                                                                  preferredStyle:UIAlertControllerStyleAlert];
    [actionSheet addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.keyboardType = UIKeyboardTypeDecimalPad;
    }];
    UIAlertAction *btnCancel = [UIAlertAction actionWithTitle:kJL_TXT("jl_cancel") style:UIAlertActionStyleCancel
                                                   handler:^(UIAlertAction * _Nonnull action) {
        kJLLog(JLLOG_DEBUG,@"点击了取消");
    }];
    UIAlertAction *btnConfirm = [UIAlertAction actionWithTitle:kJL_TXT("confirm") style:UIAlertActionStyleDefault
                                                   handler:^(UIAlertAction * _Nonnull action) {
