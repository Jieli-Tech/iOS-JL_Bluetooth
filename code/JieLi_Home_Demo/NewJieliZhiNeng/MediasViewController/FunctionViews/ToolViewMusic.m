...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.mSonyName_0.font = FontRegular(13.0);
        ...
        self.mSonyName_1.font = FontRegular(13.0);
        ...
        self.mSonyName_2.font = FontRegular(13.0);
...
-(void)showID3UI:(BOOL)isShow{
    if (isShow) {
        ...
        self.mSonyName_1.font = FontRegular(17.0);
    }
}

-(void)updateFirstUI{
    kJLLog(JLLOG_DEBUG,@"----------------------> updateFirstUI");
...
-(void)noteFirstUpdateCardMusicInfo:(NSNotification*)note{
    kJLLog(JLLOG_DEBUG,@"---> Note First Update CardMusic Info.");
    [DFAction mainTask:^{
        isMusicInfo = YES;
        [self.mProgressView setUserInteractionEnabled:YES];
        [self updateDeviceState];
...
-(void)chooseDiffMusic{
    ...
    if(model.currentFunc == JL_FunctionCodeMUSIC){
        ...
        if (isOK == NO) {
            kJLLog(JLLOG_DEBUG,@"连接中...暂不获取获取音乐信息.");
            return;
        }
...
-(void)notePlayerProgress:(NSNotification*)note{
    ...
    if (isID3_push == YES) {
        kJLLog(JLLOG_DEBUG,@"UI关闭ID3信息。100");
...
-(void)updatePhoneMusicInfo{
    ...
    if (entity){
       JLModel_Device *dev = [entity.mCmdManager getDeviceModel];
        if (dev.currentFunc == JL_FunctionCodeMUSIC){
            return;
        }
    }
...
-(void)setID3_Enable:(BOOL)enable Action:(ID3_ACTION)action{
    ...
    if (isID3_push == !enable) {
        ...
        if (enable) {
            kJLLog(JLLOG_DEBUG,@"UI开启ID3信息。22");
        }else{
            kJLLog(JLLOG_DEBUG,@"UI关闭ID3信息。22");
        }
        if (action) { action();}
    ...
    if(model.currentFunc == JL_FunctionCodeBT){
        ...
        if (isID3_ST == NO) {
            ...
            if (tp != DFAudioPlayer_TYPE_NET) {
                ...
                kJLLog(JLLOG_DEBUG,@"Musci progress : %.2f",_mProgressView.value);
        ...
        [JL_Tools mainTask:^{
                ...
                [self->devPlayer cmdPlayerCtrl:0x06 Second:(uint16_t)fabsf((pg * f_tott - f_curt)) Manager:entity.mCmdManager Result:^(JL_CMDStatus status, uint8_t sn, NSData * _Nullable data) {
                    if (sv_tott > 60*60) {
                        [JL_Tools delay:0.6 Task:^{
                            kJLLog(JLLOG_DEBUG,@"----> delay get music progess 0");
            ...
            if(pg-current>0){
                [self->devPlayer cmdPlayerCtrl:0x07 Second:(uint16_t)(pg * f_tott - f_curt) Manager:entity.mCmdManager Result:^(JL_CMDStatus status, uint8_t sn, NSData * _Nullable data) {
                    if (sv_tott > 60*60) {
                        [JL_Tools delay:0.6 Task:^{
                            kJLLog(JLLOG_DEBUG,@"----> delay get music progess 1");
                            [self getDeviceMusicProgress];
                        }];
                    }else{
                        [self getDeviceMusicProgress];
...
-(void)updateProgressTOTT:(uint32_t)n_tott CURT:(uint32_t)n_curt{
    ...
    sv_tott = tott;
    sv_curt = curt;
    kJLLog(JLLOG_DEBUG,@"update device music -----> %d",sv_curt);
...
-(void)updateID3MusicInfo:(NSNotification*)note{
    ...
    kJLLog(JLLOG_DEBUG,@"----------> showID3UI 2222");
    [self showID3UI:YES];
...
-(void)updateID3MusicStatus:(NSNotification*)note{
    ...
    if (isWaitPuase == YES && deviceModel.ID3_Status == 0) { //暂停
        ...
        [DFAction delay:3.0 Task:^{
            ...
            kJLLog(JLLOG_DEBUG,@"UI开启ID3信息。88");
        }];
    }
    ...
    kJLLog(JLLOG_DEBUG,@"playStatus:%d",deviceModel.ID3_Status);
    if (deviceModel.ID3_Status == 1) { //播放
        kJLLog(JLLOG_DEBUG,@"mul_icon_play_nor......3");
        [DFUITools setButton:btnPP Image:@"Theme.bundle/mul_icon_pause_nor"];
        
    }else{
        kJLLog(JLLOG_DEBUG,@"mul_icon_pause_nor......3");
        [DFUITools setButton:btnPP Image:@"Theme.bundle/mul_icon_play_nor"];
...
-(void)updateID3MusicPuase:(NSNotification*)note{
    ...
    if (isWaitPuase == YES) {
        ...
        [DFAction delay:1.0 Task:^{
            kJLLog(JLLOG_DEBUG,@"----------> showID3UI 3333");
            [self showID3UI:YES];
            ...
            kJLLog(JLLOG_DEBUG,@"UI开启ID3信息。99");
        }];
    }
    BOOL isID3_ST = [[JLCacheBox cacheUuid:bleSDK.mBleUUID] isID3_ST];
    if (isID3_ST && isWaitCutSong == YES) {
        isWaitCutSong = NO;
        kJLLog(JLLOG_DEBUG,@"mul_icon_play_nor......4");
...
-(void)noteShowId3:(NSNotification*)note{
    //第三方音乐，关闭设备音乐的定时器.
    [DFAction timingStop:myTimer];
    kJLLog(JLLOG_DEBUG,@"----------> showID3UI 1111");
    [self showID3UI:YES];
...
-(void)noteBleChangeMaster:(NSNotification*)note{
    ...
    if (isID3_ST == YES) {
        kJLLog(JLLOG_DEBUG,@"主从切换，恢复ID3音乐信息.");
        [[[JL_RunSDK sharedMe] mBleEntityM].mCmdManager.mChargingBinManager cmdID3_PushEnable:YES];
    }
}
...
-(void)notePlayerInterruption:(NSNotification*)note{
    ...
    if (nowPlayer.mState == DFAudio_ST_PLAYING) {
        ...
        kJLLog(JLLOG_DEBUG,@"中断正在播放的手机音乐.");
        kJLLog(JLLOG_DEBUG,@"UI开启ID3信息。77");
        ...
        [DFAction delay:0.5 Task:^{
            kJLLog(JLLOG_DEBUG,@"---> 手机音乐被打断，获取ID3信息。");
...
-(void)noteAppActive:(NSNotification*)note{
    if (bleSDK.mBleEntityM) {
        kJLLog(JLLOG_DEBUG,@"App Active reload..");
