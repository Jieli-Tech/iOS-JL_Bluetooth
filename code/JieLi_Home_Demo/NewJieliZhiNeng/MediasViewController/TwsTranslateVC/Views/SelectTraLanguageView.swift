...
class SelectTraLanguageView: BasicView {
    var currentMySideLanguage: TranslateLanguage = .zh
    var currentOtherSideLanguage: TranslateLanguage = .en
    ...
    public let currentLanguageType = PublishRelay<TranslateLanguage>()
    /// 配置语言
    /// - Parameters:
    ///   - mySide: 我方
    ///   - otherSide: 对方
    public func configLanguage(mySide: TranslateLanguage, otherSide: TranslateLanguage) {
        ...
        JLLogManager.logLevel(.DEBUG, content: "didChangeLanguage MySide:\(currentMySideLanguage) OtherSide:\(currentOtherSideLanguage)")
    }

    public func configLanguage(typeMode: TranslateLanguage) {
        if currentBtnTag {
    ...
    public func getSideLanguage() -> (TranslateLanguage, TranslateLanguage) {
