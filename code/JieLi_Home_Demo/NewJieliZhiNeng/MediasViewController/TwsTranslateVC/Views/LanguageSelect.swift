...
class LanguageSelect: BasicView {
    ...
    let languangeSelectView = SelectTraLanguageView()
    ...
    override func initData() {
        ...
        let tapGes = UITapGestureRecognizer(target: self, action: #selector(disAction))
        bgView.addGestureRecognizer(tapGes)
    }

    @objc private func disAction() {
        confirmBtn.sendActions(for: .touchUpInside)
    }
}
