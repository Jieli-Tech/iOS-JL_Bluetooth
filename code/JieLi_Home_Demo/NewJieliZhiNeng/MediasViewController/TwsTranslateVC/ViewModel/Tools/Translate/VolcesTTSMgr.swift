...
class VolcesTTSMgr: NSObject {
    ...
    private func msgFormat(_ text: String, _ uid: String) -> Data {
        ...
        var voiceType = "zh_female_tianmeitaozi_mars_bigtts"
        let recognizer = NLLanguageRecognizer()
        recognizer.processString(text)
        if recognizer.dominantLanguage == .japanese {
            voiceType = "BV522_streaming"
        }
        let audioDict: [String: Any] = ["voice_type": voiceType, "rate": 16000, "BitRate": 16, "encoding": "pcm"]
