...
class TranslateAV2Helper: NSObject, TranslateDeEncodePtl {
    ...
    required init(_ pcmResultBlock: @escaping (Data) -> Void, _ opusResultBlock: @escaping (Data) -> Void, _ pcmLRResultBlock: ((Data?, Data?) -> Void)? = nil) {
        super.init()
        self.pcmResultBlock = pcmResultBlock
        av2ResultBlock = opusResultBlock
...
extension TranslateAV2Helper: JLAV2CodecDelegate {
    func decodecData(_ data: Data?, error: (any Error)?) {
        if error != nil {
            JLLogManager.logLevel(.ERROR, content: "jlav2 decode error:\(String(describing: error))")
            return
        }
    ...
    func encodecData(_ data: Data?, error: (any Error)?) {
        if error != nil {
            JLLogManager.logLevel(.ERROR, content: "jlav2 encode error:\(String(describing: error))")
            return
        }
        guard let data = data else { return }
        av2ResultBlock?(data)
