...
class JLAudioSessionManager {
    ...
    {
        // 避免重复设置
        if isActive, session.category == category, session.categoryOptions == options { return }
    ...
    func deactivate() {
        ...
        do {
            ...
            JLLogManager.logLevel(.DEBUG, content: "AVAudioSession deactivate failed: \(error)")
        }
        isActive = false
    }
