...
-(void)setKMaxRowCount:(NSInteger) maxRowCount WithItemCountPerRow:(NSInteger) itemCountPerRow{
    ...
    UIView *cicleView = [[UIView alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/2-26/2, collectionView.frame.origin.y+collectionView.frame.size.height+21.5, 26, 8)];
...
//        }
        ...
        if([imageURL containsString:@("https")]){
            [cell.mImageView sd_setImageWithURL:[NSURL URLWithString:imageArray[indexPath.row]] placeholderImage:[UIImage imageNamed:@"Theme.bundle/img_default.png"]];
        }else{
