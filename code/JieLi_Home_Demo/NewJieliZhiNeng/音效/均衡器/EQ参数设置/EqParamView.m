...
@interface EqParamView()<LanguagePtl>{
    float sW;
    float sH;
    ...
    if (self) {
        ...
        [eqSelectView setEQSelectBLK:^(uint8_t eqMode, NSArray * _Nullable eqArray) {
            if (self->blockParam) {
                ...
                kJLLog(JLLOG_DEBUG,@"model:%@",model0);
        ...
        [sliderView outputEqArray:^(NSArray * _Nonnull eqArray) {
            ...
            if (self->blockParam) {
                ...
                kJLLog(JLLOG_DEBUG,@"setUpEqArray custom");
            ...
            //kJLLog(JLLOG_DEBUG,@"EQ Change --------------> %d %d",value,index);
            JLModel_Device *model = [self->bleSDK.mBleEntityM.mCmdManager outputDeviceModel];
        ...
        kJLLog(JLLOG_DEBUG,@"setUpEqArray 2222");
        ...
        btnEQ.titleLabel.text = kJL_TXT("eq_user");
        [btnEQ setTitle:kJL_TXT("eq_user") forState:UIControlStateNormal];
        ...
        btnRest.titleLabel.text = kJL_TXT("eq_reset");
        [btnRest setTitle:kJL_TXT("eq_reset") forState:UIControlStateNormal];
        if(model.karaokeEQType == JL_KaraokeEQTypeYES){
        ...
        btnSettings.titleLabel.text = kJL_TXT("eq_advanced_setting");
        [btnSettings setTitle:kJL_TXT("eq_advanced_setting") forState:UIControlStateNormal];
        if(model.karaokeEQType == JL_KaraokeEQTypeYES){
...
-(void)onRestBtn{
    ...
    kJLLog(JLLOG_DEBUG,@"setUpEqArray 0000");
    if (model.eqType == JL_EQType10) {
        NSArray *defaultArr = @[@(32),@(64),@(125),@(250),@(500),@(1000),@(2000),@(4000),@(8000),@(16000)];
    ...
    if (bleSDK.mBleEntityM) {
        [bleSDK.mBleEntityM.mCmdManager.mSystemEQ cmdSetSystemEQ:JL_EQModeCUSTOM Params:zoreArr];
    }else{        
        if (model.eqType == JL_EQType10) {
...
-(void)addNote{
    ...
    [JLModel_Device observeModelProperty:@"pitchLow" Action:@selector(updateHighLowVolFromDevice) Own:self];
    [JLModel_Device observeModelProperty:@"pitchHigh" Action:@selector(updateHighLowVolFromDevice) Own:self];
}
