...
typedef NS_ENUM(UInt8, ReverberationFunType) {
    ReverberationType       = 0x00, //支持混响
    DynamicType             = 0x01, //支持限幅器
};
...
-(void)handleContentUI{
    ...
    [depthLabel setFont:FontMedium(16.f)];
    ...
    [intensityLabel setFont:FontMedium(16.f)];
    ...
    [dynamicLimiterLabel setFont:FontMedium(16.f)];
...
-(void)handleDynamicLimiterValue:(IACircularSlider*)slider{
    ...
    if((model.reverberationTypes.count==2  && [model.reverberationTypes containsObject:@(ReverberationType)]) //支持混响和限幅器
       || (model.reverberationTypes.count==1  && [model.reverberationTypes containsObject:@(DynamicType)])){ //只支持限幅器
       slider.userInteractionEnabled = YES;
    }
    if(model.reverberationTypes.count==1  && [model.reverberationTypes containsObject:@(ReverberationType)]) {//只支持混响
        slider.userInteractionEnabled = NO;
    }
...
-(void)sendMessage{
    ...
    int type = -1; 
    if(model.reverberationTypes.count==2  && [model.reverberationTypes containsObject:@(ReverberationType)] //支持混响和限幅器
       && [model.reverberationTypes containsObject:@(DynamicType)]){
        type = JL_ReverberationAndDynamicType;
    }
    if(model.reverberationTypes.count==1  && [model.reverberationTypes containsObject:@(ReverberationType)]) {//只支持混响
        type = JL_OnlyReverberationType;
    }
    if(model.reverberationTypes.count==1  && [model.reverberationTypes containsObject:@(DynamicType)]) {//只支持限幅器
        type = JL_OnlyDynamicLimiterType;
    }
