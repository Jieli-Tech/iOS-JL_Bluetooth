...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    }else if(model.type == JL_DeviceTypeTWS){
    ...
    }else if (model.type == JL_DeviceTypeChargingBin){
        cell.mImageView.image = [UIImage imageNamed:@"Theme.bundle/location_icon_chargingBin"];
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *list = [[[JL_RunSDK sharedMe] mBleMultiple] bleConnectedArr];
    for (JL_EntityM *item in list) {
        if ([item.mUUID isEqualToString:objc.uuid]) {
            vc.energy = item.mCmdManager.mTwsManager.electricity;
            break;
        }
    }
