...
- (void)viewDidLoad {
    ...
    kJLLog(JLLOG_DEBUG,@"vidStr:%@",vidStr);
    kJLLog(JLLOG_DEBUG,@"pidStr:%@",pidStr);
...
- (void)viewWillAppear:(BOOL)animated{
    entity.mPower_L = _energy.powerLeft;
    entity.mPower_R = _energy.powerRight;
    [self findDevice];
    [self showSoundUI];
        ...
        [JL_Tools subTask:^{
            [self->locationManager requestLocationWithReGeocode:YES completionBlock:^(CLLocation *location, AMapLocationReGeocode *regeocode, NSError *error) {
                if (!error) {
                    ...
                    kJLLog(JLLOG_DEBUG,@"%s %s :%@",__FILE_NAME__,__func__,error);
                }
            }];
        }];
    }else{
...
-(void)startSearch{
    ...
    if(self.deviceObjc.type == JL_DeviceTypeSoundBox || self.deviceObjc.type == JL_DeviceTypeSoundCard|| self.deviceObjc.type == JL_DeviceTypeChargingBin){
        ...
        if(self.deviceObjc.type == JL_DeviceTypeSoundBox || self.deviceObjc.type == JL_DeviceTypeSoundCard || self.deviceObjc.type == JL_DeviceTypeChargingBin){
            for (NSString *key in pointDict) {
                ...
                if ([dict[@"lat"] isEqual:userLocation[@"lat"]] && [dict[@"long"] isEqual:userLocation[@"long"]]) {
                    ...
                    }else if (objc.type == JL_DeviceTypeChargingBin){
                        annotationView.image = [UIImage imageNamed:@"Theme.bundle/location_icon_chargingBin"];
                    }
                }
            }
...
-(void)initBottomViews{
    ...
    if(self.deviceObjc.type == JL_DeviceTypeTWS){ //1:耳机
        ...
        NSString *defaultImg = [ImageCacheUtil getProductImageEarphoneLeft:self.deviceObjc.type];
        [ImageCacheUtil setImageView:leftImv DeviceUuid:self.deviceObjc.uuid Image:@"LEFT_DEVICE_CONNECTED" Default:defaultImg];
        ...
        NSString *defaultRight = [ImageCacheUtil getProductImageEarphoneRight:self.deviceObjc.type];
        [ImageCacheUtil setImageView:rightImv DeviceUuid:self.deviceObjc.uuid Image:@"RIGHT_DEVICE_CONNECTED" Default:defaultRight];
