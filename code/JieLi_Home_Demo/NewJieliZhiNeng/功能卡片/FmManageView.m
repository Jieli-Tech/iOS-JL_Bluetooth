...
@interface FmManageView()<LanguagePtl>{
    float sW;
    float sH;
    BOOL  isManger;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        labelCollection = [UILabel new];
        if([kJL_GET hasPrefix:@"zh"]){
            labelCollection.frame = CGRectMake(20.0, 10.0, 80.0, 20.0);
        }else{
            labelCollection.frame = CGRectMake(20.0, 10.0, 200.0, 20.0);
        }
        labelCollection.font = FontRegular(14.0);
        labelCollection.textColor = [UIColor blackColor];
        labelCollection.text = kJL_TXT("freq_point_collect");
        [self addSubview:labelCollection];
        
        btnManager = [UIButton new];
        btnManager.frame = CGRectMake(sW-75.0, 10.0, 80, 20.0);
        btnManager.titleLabel.font = FontRegular(14.0);
        btnManager.titleLabel.textAlignment = NSTextAlignmentRight;
        [btnManager setTitle:kJL_TXT("freq_point_manage") forState:UIControlStateNormal];
        [btnManager setTitleColor:kDF_RGBA(67, 141, 255, 1.0) forState:UIControlStateNormal];
...
-(void)onManagerAciton{
    if (isManger) {
        [btnManager setTitle:kJL_TXT("device_paired_finish") forState:UIControlStateNormal];
        isManger = NO;
    }else{
        [btnManager setTitle:kJL_TXT("freq_point_manage") forState:UIControlStateNormal];
        isManger = YES;
...
-(CGSize)setFmArray:(NSArray*)array CurrentFM:(uint16_t)fm{
    ...
    for (int i = 0; i < dataArray.count; i++) {
        ...
        btn.titleLabel.font = FontRegular(14.0);
        ...
        btnDel.titleLabel.font = FontRegular(14.0);
        btnDel.tag = i+250;
//        if (i == fm_index) {
//            btnDel.hidden = YES;
//        }else{
