...
+(void)createTableBy:(FMDatabase *)db{
    ...
    if ([db open]) {
           ...
           if (result) {
               [db close];
               //kJLLog(JLLOG_DEBUG,@"t_device_image tableview create OK");
           }else{
               [db close];
               kJLLog(JLLOG_DEBUG,@"t_device_image tableview create failed");
           }
        [db close];
    }
...
+(void)updateDeviceImage:(DeviceImageModel *)model forUUID:(NSString *)uuid by:(FMDatabase *)db{
     ...
     if ([db open]) {
           ...
           if (array.count==1) {
               ...
               if (result) {
                   //kJLLog(JLLOG_DEBUG,@"device_image update OK");
               }else{
                   kJLLog(JLLOG_DEBUG,@"device_image update Failed");
               }
           }else{
               ...
               if (result) {
                   //kJLLog(JLLOG_DEBUG,@"device_image install OK");
               }else{
                   kJLLog(JLLOG_DEBUG,@"device_image install Failed");
               }
           }
...
+(void)deleteDeviceImageByUUid:(NSString *)uuid by:(FMDatabase *)db{
    ...
    if ([db open]) {
          ...
          if (result) {
              kJLLog(JLLOG_DEBUG,@"delete device_image by uuid OK");
          }else{
              kJLLog(JLLOG_DEBUG,@"delete device_image by uuid failed");
          }
          [db close];
      }
