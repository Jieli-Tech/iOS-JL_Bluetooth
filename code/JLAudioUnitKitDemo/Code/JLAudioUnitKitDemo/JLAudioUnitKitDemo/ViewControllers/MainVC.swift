...
class MainVC: BaseViewController {
    ...
    override func initUI() {
        ...
        mp3ToUmp3Btn.setTitle("MP3 To UMP3", for: .normal)
        mp3ToUmp3Btn.setTitleColor(.white, for: .normal)
        mp3ToUmp3Btn.backgroundColor = UIColor.random()
        mp3ToUmp3Btn.layer.cornerRadius = 8
        mp3ToUmp3Btn.layer.masksToBounds = true
        ...
        mp3ToUmp3Btn.snp.makeConstraints { make in
            make.left.right.equalTo(view).inset(20)
            make.height.equalTo(40)
            make.top.equalTo(opusToOggBtn.snp.bottom).offset(10)
        }
    ...
    override func initData() {
        ...
        mp3ToUmp3Btn.rx.tap.subscribe(onNext: { [weak self] _ in
            guard let self = self else { return }
            self.navigationController?.pushViewController(Mp3ToUmp3VC(), animated: true)
        }).disposed(by: disposeBag)
