...
class BleManager: NSObject {
    ...
    override init() {
        ...
        assist.mDelegate = self
    }

    /// 设置是否启用设备认证
    /// 若关闭，需要多方一起关闭包括设备端跟安卓端
    /// - Parameter status: 设备认证开关
    func setAuthEnable(_ status: Bool) {
        SettingInfo.saveAuthEnable(status)
...
extension BleManager: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateNotificationStateFor characteristic: CBCharacteristic, error _: Error?) {
        if SettingInfo.getAuthEnable() {
            ...
            if SettingInfo.getAuthEnable() == false {
    ...
    func peripheral(_ cbp: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error err: Error?) {
        ...
        if currentEntity == nil && SettingInfo.getAuthEnable() == false {
            JLLogManager.logLevel(.DEBUG, content: "receiveData:\(characteristic.value?.eHex ?? "")")
            return
        }
    ...
    private func initTargetFeature(peripheral: CBPeripheral) {
        ...
        currentCmdMgr?.cmdTargetFeatureResult { state,_,_ in
            ...
            self.connectTimeoutBlock?(true)
            // 保存历史
            let isAtt = self.discoverAttDevice[peripheral.identifier.uuidString] == nil ? false : true
            self.isConnectWithAtt = isAtt
            JLLogManager.logLevel(.DEBUG, content: "discoverAttDevice:\(self.discoverAttDevice)")
            DevHistory.share.insert(peripheral, entity.mAdvData, isAtt)
            JL_Tools.post(kJL_BLE_M_ENTITY_CONNECTED, object: self.handleCbp)
            self.connectTimeoutBlock = nil
            TimerHelper.stopTimer(timerID: &self.connectTimeoutID)
...
extension BleManager: JL_AssistDelegate {
    func assistDidWrite(_ data: Data) {
        ...
        while offset < data.count {
            let seek = min(mtu, data.count - offset)
            let chunk = data.subdata(in: offset..<offset+seek)
            peripheral.writeValue(chunk, for: write, type: .withoutResponse)
            offset += seek
        }
    }
}
