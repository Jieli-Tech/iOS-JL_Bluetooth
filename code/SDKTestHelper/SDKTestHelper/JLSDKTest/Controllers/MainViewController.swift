...
class MainViewController: BaseViewController {
    ...
    var itemsArray: [String] = [R.localStr.fileTransport(), R.localStr.defaultSet(), R.localStr.customerCommand(), R.localStr.update(), R.localStr.localFileBrowsing(),R.localStr.laboratory()]

    override func viewDidLoad() {
        super.viewDidLoad()
    }

    private func pushHidingTabBar(_ vc: UIViewController) {
    ...
    override func initData() {
        ...
        navigationView.leftBtn.rx.tap.subscribe(onNext: { [weak self] in
            let vc = SearchBleViewController()
            self?.pushHidingTabBar(vc)
        ...
        historyBtn.rx.tap.subscribe { [weak self] _ in
            let vc = DevHistoryViewController()
            self?.pushHidingTabBar(vc)
        }.disposed(by: disposeBag)
    }
    ...
    @objc func connectStatusChange(_: Notification) {
        if (BleManager.shared.currentEntity) != nil {
            ...
            self.itemsArray = [R.localStr.fileTransport(), R.localStr.defaultSet(), R.localStr.customerCommand(), R.localStr.update(), R.localStr.localFileBrowsing(),R.localStr.laboratory()]
            subFuncTable.reloadData()
        } else {
            self.itemsArray = [R.localStr.customerCommand()]
            navigationView.leftBtn.setTitle(R.localStr.search(), for: .normal)
        }
        subFuncTable.reloadData()
    }
...
extension MainViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0, BleManager.shared.currentPeripheral != nil {
            ...
            case 0:
                pushHidingTabBar(FileTransportViewController())
            case 1:
                pushHidingTabBar(DefaultSetViewController())
            case 2:
                pushHidingTabBar(CustomCmdVC())
            case 3:
                pushHidingTabBar(UpdatesViewController())
            case 4:
                pushHidingTabBar(BrowseViewController())
            case 5:
                pushHidingTabBar(TestUnitViewController())
            default:
                break
            }
            return 
