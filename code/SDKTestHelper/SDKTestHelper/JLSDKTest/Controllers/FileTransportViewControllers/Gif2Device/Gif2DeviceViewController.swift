...
class Gif2DeviceViewController: BaseViewController {
    ...
    private var packageType: JLGIFBinPackageType = .JLVGL
        ...
        confirmBtn.rx.tap.subscribe { [weak self] _ in
            ...
            if targetFile.count > 0 {
                ...
                if let dt = try? Data(contentsOf: URL(fileURLWithPath: targetFile)) {
                    JLGifBin.makeData(toBin: dt, level: Int32(level), chipType: mode, packageType: packageType) { [weak self] _, data in
                        guard let `self` = self else { return }
    ...
    private func modePackageTypeViewInit() {
        modePackageTypeView.title = "Package Type"
        let items = ["JLVGL","LVGL"]
    ...
    private func namePath()->String {
        let chipName = mode == .JL_701N ? "JL701N" : "JL707N"
        let levelName = level == 1 ? "low" : level == 2 ? "middle" : "high"
        let type = packageType == .JLVGL ? "JLVGL" : "LVGL"
        let name = chipName + "_" + levelName + "_" + type
        let path = _R.path.gif2Rgb + "/" + name + ".bin"
        return path
    }
    private func saveData(){
        guard let data = self.gifBin else { return }
        let path = namePath()
