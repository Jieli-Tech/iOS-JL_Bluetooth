...
class DialTpViewController: BaseViewController, UIImagePickerControllerDelegate & UINavigationControllerDelegate {
    let sourceFlashSizeLab = UILabel()
    ...
    private var observerCurrentDial: NSKeyValueObservation?
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        observerCurrentDial?.invalidate()
    }
    ...
    override func initUI() {
        ...
        sourceFlashSizeLab.text = ""
        sourceFlashSizeLab.textColor = UIColor.black
        sourceFlashSizeLab.font = UIFont.boldSystemFont(ofSize: 15)
        sourceFlashSizeLab.textAlignment = .center
        ...
        view.addSubview(sourceFlashSizeLab)
        ...
        sourceFlashSizeLab.snp.makeConstraints { make in
        ...
        seleteFileBtn.snp.makeConstraints { make in
            make.top.equalTo(sourceFlashSizeLab.snp.bottom).offset(10)
    ...
    private func initDial() {
        ...
        chain.addTask { [weak self] _, completion in
            guard let self = self else { return }
            dialMgr?.getFileList(devMode.currentCardType, count: 100, completion: { _ , err in
                completion(nil, nil)
            })
        }
        ...
        chain.addTask({ [weak self] _, completion in
            guard let self = self else { return }
            dialMgr?.getFlashRemainingSpace { size, err in
                if err != nil {
                    completion(nil, err)
                }else{
                    self.sourceFlashSizeLab.text = "剩余空间:\(size)byte"
                    completion(nil, nil)
                }
            }
        })
        chain.run(withInitialInput: nil) { [weak self] _, err in
            ...
            if (err != nil) {
                ...
                observerCurrentBg = dialMgr?.observe(\.currentBackground, options: .new, changeHandler: { _, value in
                    guard let value = value.newValue else { return }
                    value?.logProperties()
                })
                observerCurrentDial = dialMgr?.observe(\.currentFileSource, options: .new, changeHandler: { _, value in
                    guard let value = value.newValue else { return }
                    value.logProperties()
                })
            }
