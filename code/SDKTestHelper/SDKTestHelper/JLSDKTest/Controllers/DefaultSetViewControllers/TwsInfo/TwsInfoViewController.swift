...
class TwsInfoViewController: BaseViewController {
    ...
    // 设置高低音
    private func bassBoostSet() {
        guard let mManager: JL_ManagerM = BleManager.shared.currentCmdMgr else { return }
        mManager.mChargingBinManager.cmdSetLowPitch(12, highPitch: 12)
    }
    
    private func  bassBoostGet() {
        guard let mManager: JL_ManagerM = BleManager.shared.currentCmdMgr else { return }
        mManager.cmdGetSystemInfo(.COMMON, selectionBit: 0x0800) { status, _, _ in
            if status == .success {
                let model = mManager.getDeviceModel()
                JLLogManager.logLevel(.DEBUG, content: "bassBoost: \(model.pitchLow), \(model.pitchHigh)")
            }
        }
    }
    
    private func systemVolumeGet() {
        guard let mManager: JL_ManagerM = BleManager.shared.currentCmdMgr else { return }
        mManager.cmdGetSystemInfo(.COMMON, selectionBit: 0x0002) { status, _, _ in
            if status == .success {
                let model = mManager.getDeviceModel()
                JLLogManager.logLevel(.DEBUG, content: "systemVolume: \(model.currentVol)")
            }
