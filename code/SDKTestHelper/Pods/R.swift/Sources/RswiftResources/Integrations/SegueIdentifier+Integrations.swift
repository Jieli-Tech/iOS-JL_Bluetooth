...
import Foundation
    ...
    public extension SeguePerformer {
         ...
         */
        func performSegue<Segue, Destination>(withIdentifier identifier: SegueIdentifier<Segue, Self, Destination>, sender: Any?) {
            performSegue(withIdentifier: identifier.identifier, sender: sender)
        }
    }

    public extension SegueIdentifier where Segue: UIStoryboardSegue {
        /// Optionally returns a typed version of the segue.
        /// Returns nil if either the segue identifier, the source, destination, or segue types don't match.
        /// For use inside `prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?)`.
        func callAsFunction(segue: Segue) -> TypedSegue<Segue, Source, Destination>? {
            TypedSegue(segueIdentifier: self, uiStoryboardSegue: segue)
        }
    }

    public extension SegueIdentifier where Segue: UIStoryboardSegue, Source: UIViewController, Destination: UIViewController {
        /// Trigger a segue by providing a source, destination and handler
        func perform(source: Source, destination: Destination, handler: @escaping () -> Void) {
    ...
    public extension TypedSegue {
         ...
         */
        init?<StoryboardSegue: UIStoryboardSegue>(segueIdentifier: SegueIdentifier<Segue, Source, Destination>, uiStoryboardSegue: StoryboardSegue) {
            guard
