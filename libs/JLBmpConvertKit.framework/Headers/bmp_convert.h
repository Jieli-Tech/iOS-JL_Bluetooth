 ...
 *     1 : 转换成功
 */
int br23_bmp_to_res_path(char *infile, int width, int height, char *outfile);
 ...
 *     1 : 转换成功
 */
int br28_bmp_to_res_path(char *infile, int width, int height, char *outfile);
 ...
 *     1 : 转换成功
 */
int br28_bmp_to_res_path_nopack(char *infile, int width, int height, char *outfile);
 ...
 *    转换后的数据大小
 */
int br28_png_to_res(u8 *inbuf, u32 inWidth, u32 inHeight, u8 packet, u8 *outbuf, u32 outsize);
...
int br28_png_to_res_path(char *infile, int width, int height, char *outfile);
 ...
 *     1 : 转换成功
 */
int br28_png_to_res_path_nopack(char *infile, int width, int height, char *outfile);
 ...
 *    转换后的数据大小
 */
int br35_bmp_to_res(u8 *inbuf, u32 inWidth, u32 inHeight, u8 packet, u8 *outbuf, u32 outsize);
 ...
 *     1 : 转换成功
 */
int br35_bmp_to_res_path(char *infile, int width, int height, char *outfile);
 ...
 *     1 : 转换成功
 */
int br35_bmp_to_res_path_nopack(char *infile, int width, int height, char *outfile);
 ...
 *    packet   : 数据是否打包 
 *输出：
 *    outbuf   : 存放转换后的数据缓存，为NULL时计算需要的缓存大小(图像为ARGB8565格式, 杰理打包格式)
 *    outsize  : 数据缓存大小
 *返回值   ：
 *    转换后的数据大小
 */
int br35_png_to_res(u8 *inbuf, u32 inWidth, u32 inHeight, u8 packet, u8 *outbuf, u32 outsize);
 ...
 *     height : bmp图像高度
 * 输出：
 *     outfile : 输出文件路径(转换后的文件)(图像为ARGB8565格式, 杰理打包格式)
 * 返回值：
 *     0 : 转换失败(输入的width, height与infile文件的大小不匹配)
 *     1 : 转换成功
 */

int br35_png_to_res_path(char *infile, int width, int height, char *outfile);
 ...
 *     1 : 转换成功
 */
int br35_png_to_res_path_nopack(char *infile, int width, int height, char *outfile);
