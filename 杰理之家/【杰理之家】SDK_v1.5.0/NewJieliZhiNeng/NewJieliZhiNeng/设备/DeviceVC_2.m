...
@interface DeviceVC_2 ()<UITableViewDelegate,UITableViewDataSource>{
...
-(void)setupUI{
    ...
    [subBtn_2 setTitle:kJL_TXT("添加设备") forState:UIControlStateNormal];
    subTitleView_H.constant = kJL_HeightNavBar;
    noDeviceLabel.text = kJL_TXT("暂无设备，请先连接设备");
...
-(void)updateDataUI{
    ...
    [JL_Tools delay:0.2 Task:^{
        if (self->bleSDK.mBleEntityM) {
            /*--- 刷新卡片数据界面 ---*/
            NSLog(@"---> 获取耳机信息... 1");
            [self->bleSDK.mBleEntityM.mCmdManager cmdHeatsetGetAdvFlag:0x3F
                                            Result:^(NSDictionary * _Nullable dict) {
                bleDeviceDict = dict;
            }];
        }
