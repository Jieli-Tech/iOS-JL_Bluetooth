...
-(void)scrollAction:(UIScrollView*)scrollView{
    if (isSpeed == NO) {
        ...
        NSLog(@"无速度：%.2f,%d",s_x,k);
        if (k>self.maxValue) {
            int currentValue;
            if(self.maxValue%100<100){
                currentValue = (self.maxValue/100)+1;
            }else{
                currentValue = self.maxValue/100;
            }
...
//        NSInteger outPoint = (NSInteger)fm_point*100.0;
...
-(NSInteger)calcutePoint:(int)point{
    ...
    NSLog(@"fixedOffset_0:%.2f",fm_point*100.0);
    return fm_point*100.0;
}
...
-(void)fixedOffset_1:(int)point{
    ...
    NSLog(@"fixedOffset_1:%.2f",fm_point*100.0);
    if(mType == 0){
