...
-(void)updateAlertInfo{
    ...
    if([kJL_GET hasPrefix:@"zh"]){
        ...
        if(_funType == 2){
            ...
            [cancelBtn setTitle:@"下次开机生效" forState:UIControlStateNormal];
        }
        ...
        if(_funType == 2){
            ...
            [finishBtn setTitle:@"立即生效" forState:UIControlStateNormal];
        }
        robotTip1.adjustsFontSizeToFitWidth = NO;
        robotTip1.minimumScaleFactor = 1.0;
        NSString *robotTip1Str;
        if(_funType ==0){
            robotTip1Str = @"该操作需要重启设备才能生效,";
        }
        if(_funType ==1){
            robotTip1Str = @"设备配置信息有更新,";
        }
        if(_funType ==2){
            robotTip1Str = @"是否要忽略此设备?";
        }
        if (@available(iOS 13.0, *)) {
        ...
        }else if([DFUITools screen_2_W] ==812.0){
            robotTip3.frame = CGRectMake(14,robotTip2.frame.origin.y+18+2,centerView.frame.size.width-14*2,18);
        }else{
        ...
        if (@available(iOS 13.0, *)) {
            ...
            NSMutableAttributedString *robotTip2_Str = [[NSMutableAttributedString alloc] initWithString:@"是否立即生效？" attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 14],NSForegroundColorAttributeName: myColor}];

            robotTip2.attributedText = robotTip2_Str;
        } else {
            // Fallback on earlier versions
            NSMutableAttributedString *robotTip2_Str = [[NSMutableAttributedString alloc] initWithString:@"是否立即生效？" attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 14],NSForegroundColorAttributeName: [UIColor colorWithRed:36/255.0 green:36/255.0 blue:36/255.0 alpha:1.0]}];
        ...
        if (@available(iOS 13.0, *)) {
            ...
            NSMutableAttributedString *robotTip3_Str = [[NSMutableAttributedString alloc] initWithString:@"(若未能正确显示设备名称，请重新配对连接)" attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 12],NSForegroundColorAttributeName: myColor}];

            robotTip3.attributedText = robotTip3_Str;
        } else {
            // Fallback on earlier versions
            NSMutableAttributedString *robotTip3_Str = [[NSMutableAttributedString alloc] initWithString:@"(若未能正确显示设备名称，请重新配对连接)" attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 12],NSForegroundColorAttributeName: [UIColor colorWithRed:126/255.0 green:126/255.0 blue:126/255.0 alpha:1.0]}];
...
-(void)initUI{
    ...
    if (@available(iOS 13.0, *)) {
        ...
        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:kJL_TXT("提 示") attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 15],NSForegroundColorAttributeName: titleColor}];
        
        titleLab.attributedText = string;
    } else {
        // Fallback on earlier versions
        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:kJL_TXT("提 示") attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 15],NSForegroundColorAttributeName: [UIColor colorWithRed:36/255.0 green:36/255.0 blue:36/255.0 alpha:1.0]}];
