...
-(void)initWithData{
    ...
    itemArray = @[kJL_TXT("当前版本"),kJL_TXT("固件升级")];
    ...
    if (model.md5Type == YES) {
        ...
        [self.otaEntity.mCmdManager cmdGetMD5_Result:^(NSArray * _Nullable array) {
            if (array.count >= 3) {
                ...
                [DFUITools showText_1:@"设备MD5校验失败." onView:self.view delay:1.0];
                self->isChecking = NO;
                [self.upgradeTable reloadData];
            }
        }];
...
-(void)updateWithOTAResult:(JL_OTAUrlResult)result
    ...
    if(result == JL_OTAUrlResultDownloadFail){
        [DFUITools showText_1:kJL_TXT("当前网络不可用") onView:self.view delay:1.0];
        self->isChecking = NO;
        self->pRate = 0;
    ...
    if (result == JL_OTAUrlResultFail) {
        [DFUITools showText_1:kJL_TXT("服务器异常，未能获取升级文件。") onView:self.view delay:1.0];
        self->isChecking = NO;
        [self->_upgradeTable reloadData];
        return;
    }
    ...
    if([DeviceInfoTools shouldUpdate:version local:currentFireCode]){
        ...
        self->shouldUp = NO;
        [DFUITools showText_1:kJL_TXT("暂无新版本") onView:self.view delay:1.0];
    }
    self->isChecking = NO;
    [self->_upgradeTable reloadData];
}
...
-(void)initWithUI{
    ...
    self.titleLab.text = kJL_TXT("固件更新");
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 1) {
        ...
        if (shouldUp) {
            ...
            [cell.downloadBtn setTitle:kJL_TXT("升级") forState:UIControlStateNormal];
...
#endif
    [self.upgradeTable reloadData];
    [DFUITools showText:kJL_TXT("当前网络不可用") onView:self.view delay:2.0];
    pRate = 0;
}
...
-(void)upgradeAction{
    ...
    if(data.length >0){
        [transportView update:0.0 Text:kJL_TXT("正在升级")];
...
-(void)noteEnterBackground:(NSNotification*)note{
    ...
    if (isUpdateing) {
        //命令退出OTA
        [self.otaEntity.mCmdManager cmdOTACancelResult:nil];
        ...
        [self failedWithAction:kJL_TXT("用户异常操作，升级失败！")];
        [self otaTimeClose];
    }
}
...
-(void)otaTimeAdd{
    ...
    if (otaTimeout == 7) {
        [self otaTimeClose];
        [self failedWithAction:kJL_TXT("OTA升级超时")];
        NSLog(@"OTA ---> 超时了！！！");
