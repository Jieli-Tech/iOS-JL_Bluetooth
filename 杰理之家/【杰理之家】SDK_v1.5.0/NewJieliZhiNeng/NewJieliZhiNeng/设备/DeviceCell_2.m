...
@interface DeviceCell_2(){
    ...
    __weak IBOutlet UILabel *subLb_Using;
...
-(void)setMSubObject:(DeviceObjc *)mSubObject{
    ...
    if (type == 1) {
        ...
        if (uuidType == JLUuidTypeDisconnected ||
            ...
            subLb_1.text = [NSString stringWithFormat:@"%@ 右",_mSubObject.name];
            subLb_2.text = [NSString stringWithFormat:@"%@ 充电仓",_mSubObject.name];
    ...
    if (uuidType == JLUuidTypeInUse) {
        ...
        subLb_Using.text = kJL_TXT("• 正在使用");
    }
    if (uuidType == JLUuidTypePreparing) {
        subLb_Using.hidden = NO;
        subLb_Using.text = kJL_TXT("• 配置中...");
    }
    if (uuidType == JLUuidTypeNeedOTA) {
        subLb_ST.hidden = NO;
        subLb_ST.text = kJL_TXT("• 需要升级");
    ...
    if (uuidType == JLUuidTypeDisconnected) {
        subLb_ST.hidden = NO;
        subLb_ST.text = kJL_TXT("• 未连接");
        subLb_ST.textColor = [UIColor darkGrayColor];
