...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [JL_Tools add:kJL_BLE_OFF Action:@selector(noteEntityDisconnected:) Own:self];
...
-(void)initWithAllUI{
    ...
    [checkBtnC setTitle:kJL_TXT("查看") forState:UIControlStateNormal];
    ...
    [finishBtnC setTitle:kJL_TXT("完成") forState:UIControlStateNormal];
...
- (IBAction)checkBtnAction:(id)sender {
    ...
    if (type == JLUuidTypeDisconnected) {
        /*--- 去连接设备 ---*/
        NSLog(@"---> 【详情】连接设备：%@",entity.mItem);
        [self startLoadingView:kJL_TXT("正在连接") Delay:15];
...
-(void)showAnimation{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        CGRect frame = CGRectMake(6, [UIScreen mainScreen].bounds.size.height-self->connectOKView.frame.size.height-6, self->connectOKView.frame.size.width, self->connectOKView.frame.size.height);
    ...
    [entity.mCmdManager cmdRequestDeviceImageVid:vidStr Pid:pidStr
        ...
        [JL_Tools mainTask:^{
            if (dict) {
                ...
                NSLog(@"---->Show Device Image!");
            }else{
                [wself setDefaultImage:entity];
                NSLog(@"---->Show Default Image.1");
            }
            if (result) {result();}
        }];
    }];
...
-(void)setDefaultImage:(JL_EntityM*)entity{
    ...
    if (entity.mType == 0x00) self->centerHeadSetImgv.image = [UIImage imageNamed:@"Theme.bundle/img_speaker"];
    if (entity.mType == 0x01) self->centerHeadSetImgv.image = [UIImage imageNamed:@"Theme.bundle/img_earphone_empty"];
    if (entity.mType == 0x04) self->centerHeadSetImgv.image = [UIImage imageNamed:@"Theme.bundle/img_mic"];
}
...
-(void)dismissView{
    ...
    if (nowEntity.mBLE_IS_PAIRED == YES) {
        ...
        //NSLog(@"---> Close Heatset Adv Enable【NO】");
    }
...
-(void)connectingBack:(JL_EntityM *)entity{
    ...
    if ([[SqliteManager sharedInstance] isExit:entity]) {//是否已连接过
        ...
        if (entity.mType == 0x01) {
            ...
            tipsLab.hidden = NO;
            NSString *showText = kJL_TXT("请确保耳机处于可配对状态");
            if (netDict) {
                NSDictionary *dict = netDict[@"device"];
                int type = [dict[@"has_charging_bin"] intValue];
                if (type == 1) {
                    showText = kJL_TXT("请长按充电仓按键进行连接");
                }else{
                    showText = kJL_TXT("请确保耳机处于可配对状态");
                }
            }
...
-(void)canNotConnect:(JL_EntityM *)entity{
    ...
    tipsLab.hidden = NO;
    tipsLab.text = kJL_TXT("设备不可连接，请长按配对按键重新连接蓝牙");
        ...
        if ([entity.mPeripheral.identifier.UUIDString isEqualToString:item.uuid]) {
            ...
            [self->bleSDK.mBleMultiple connectEntity:entity Result:^(JL_EntityM_Status status) {
                ...
                if (status == JL_EntityM_StatusPaired) {
                    ...
                    //NSLog(@"---> Open Heatset adv Enable【YES】");
                }
