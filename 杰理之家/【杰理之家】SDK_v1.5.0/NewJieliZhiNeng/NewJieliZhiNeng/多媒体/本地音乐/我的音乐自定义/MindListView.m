...
-(void) initUI{
    ...
    NSMutableAttributedString *label_1_Str = [[NSMutableAttributedString alloc] initWithString:kJL_TXT("全部歌曲") attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 18],NSForegroundColorAttributeName: [UIColor colorWithRed:36/255.0 green:36/255.0 blue:36/255.0 alpha:1.0]}];
...
-(void)getMusicList{
        [DFAction mainTask:^{
                ...
                if (self->_mItemArray > 0) {
                    ...
                    NSMutableAttributedString *label_2_Str = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:kJL_TXT("(共%lu首)"),(unsigned long)self->_mItemArray.count] attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 13],NSForegroundColorAttributeName:  [UIColor colorWithRed:90/255.0 green:90/255.0 blue:90/255.0 alpha:1.0]}];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([addr isEqualToString:entity.mEdr]) {
        ...
        [DFUITools showText:kJL_TXT("请连对应设备的蓝牙!") onView:self delay:1.0];
    }
}
...
-(void)playMusic:(NSIndexPath *)indexPath{
    ...
    [bleSDK.mBleEntityM.mCmdManager cmdID3_PushEnable:NO];
