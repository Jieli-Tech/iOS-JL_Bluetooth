...
-(void)initWithData{
    ...
    if (_headsetDict[@"MIC_MODE"]) {
        ...
        mic.funType = 3;
        NSLog(@"MIC_MODE:%@",_headsetDict[@"MIC_MODE"]);
        if ([_headsetDict[@"MIC_MODE"] intValue] == 1) {
            ...
            if (bleType == JL_DeviceTypeTWS) {
                ...
                NSLog(@"mic.detailStr:%@",mic.detailStr);
            }
            if (deviceDic) {
                mic.detailStr = [DeviceInfoTools mic_channel:2 withKey:@"mic_channel" basicDict:deviceDic];
            }
...
-(void)initWithUI{
    ...
    if (deleteBtn) {
        ...
        if (bleType == JL_DeviceTypeSoundBox || bleType == JL_DeviceTypeSoundCard) {
            deleteBtn = [[UIButton alloc] initWithFrame:CGRectMake(0,[UIScreen mainScreen].bounds.size.height-185, width, dHeight)];
        }
        if (bleType == JL_DeviceTypeTWS) {
...
-(void)headSetDenoiseMore:(JLModel_ANC *)deviceModelAnc{
    ...
    NSLog(@"DenoiseVC:left:%d,right:%d",deviceModelAnc.mAncCurrent_L,deviceModelAnc.mAncCurrent_R);
    [self presentViewController:vc animated:YES completion:nil];
}
