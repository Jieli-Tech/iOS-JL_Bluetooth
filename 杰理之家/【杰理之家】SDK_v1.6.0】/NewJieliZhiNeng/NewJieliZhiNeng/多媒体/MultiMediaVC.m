...
@interface MultiMediaVC ()<ToolViewMusicDelegate,ToolViewNetworkDelegate,LanguagePtl>{
    TopView         *topView;
        ...
        if (index == 1) {
            ...
            if (funcEnable == 1) {
                if (![deviceModel.cardArray containsObject:@(JL_CardTypeUSB)]) {
                    [DFUITools showText:kJL_TXT("usb_offline") onView:self.view delay:1.0];
                    return;
                }
            }
        ...
        if (index == 2) {
            ...
            if (funcEnable == 1) {
                if (![deviceModel.cardArray containsObject:@(JL_CardTypeSD_0)] &&
                    ...
                    [DFUITools showText:kJL_TXT("sd_offline") onView:self.view delay:1.0];
                    return;
                }
            }
        ...
        if (index == 5) {
            if (funcEnable == 1) {
                if (![deviceModel.cardArray containsObject:@(JL_CardTypeLineIn)]) {
                    [DFUITools showText:kJL_TXT("linein_offline") onView:self.view delay:1.0];
                    return;
                }
            }
...
-(NSArray*)functionViewArray:(NSArray*)enArr{
    NSArray *nameArr  = @[kJL_TXT("multi_media_local"),kJL_TXT("multi_media_usb_drive"),kJL_TXT("multi_media_sd_card"),
                          kJL_TXT("multi_media_fm_launch"),kJL_TXT("multi_media_fm_receive"),kJL_TXT("multi_media_line_in"),
                          kJL_TXT("multi_media_light_settings"),kJL_TXT("default_alarm_name"),kJL_TXT("multi_media_search_device"),
                          kJL_TXT("multi_media_net_radio"),kJL_TXT("multi_media_sound_card"),kJL_TXT("multi_media_watch")];
...
-(void)addNote{
    ...
    [JLModel_Device observeModelProperty:@"currentCard" Action:@selector(noteCurrentCard:) Own:self];//当前发射点
    
}

-(void)noteCurrentCard:(NSNotification*)note{
    BOOL isOK = [JL_RunSDK isCurrentDeviceCmd:note];
    if (isOK == NO) return;
    JLModel_Device *model = [[[JL_RunSDK sharedMe] mBleEntityM].mCmdManager outputDeviceModel];
    if (model.devPinMultiplex) {
        switch (model.currentCard) {
            case JL_CardTypeUSB:{
                [DFAction mainTask:^{
                    [self->functionsView setFunctionsViewSelectName:kJL_TXT("multi_media_usb_drive")];
                }];
            }break;
            case JL_CardTypeSD_0:
            case JL_CardTypeSD_1:{
                [DFAction mainTask:^{
                    [self->functionsView setFunctionsViewSelectName:kJL_TXT("multi_media_sd_card")];
                }];
            }break;
            default:
                break;
