...
-(void)initWithData{
    ...
    for (NSNumber *number in _cardArray) {
        switch ([number intValue]) {
            case JL_CardTypeUSB:{
                [newArray addObject:kJL_TXT("multi_media_usb_drive")];
            }break;
            case JL_CardTypeSD_0:{
                [newArray addObject:kJL_TXT("multi_media_sd_card")];
            }break;
            case JL_CardTypeSD_1:{
                [newArray addObject:kJL_TXT("multi_media_sd_card_2")];
            }break;
            default:
                break;
        }
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    JL_EntityM *entity = [[JL_RunSDK sharedMe] mBleEntityM];
    [entity.mCmdManager.mAlarmClockManager cmdRtcAudition:self.rtcModel Option:NO result:nil];
}
...
-(void)ringSelect:(RingSelectView *)view{
    ...
    JL_EntityM *entity = [[JL_RunSDK sharedMe] mBleEntityM];
    [entity.mCmdManager.mAlarmClockManager cmdRtcAudition:self.rtcModel Option:YES result:nil];
}
