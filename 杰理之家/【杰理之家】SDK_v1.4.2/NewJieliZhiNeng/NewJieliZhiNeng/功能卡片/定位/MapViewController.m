...
-(void)findDevice{
    ...
    if (entity && [uuidNow isEqual:uuidSelected]) {
        locationLab.text = kJL_TXT("正在定位中...");
        [JL_Tools subTask:^{
            [self->locationManager requestLocationWithReGeocode:YES completionBlock:^(CLLocation *location, AMapLocationReGeocode *regeocode, NSError *error) {
                if (!error) {
                    [JL_Tools mainTask:^{
                        ...
                        NSData *data;
                        if(self.deviceObjc.type == 0 || self.deviceObjc.type == 4){
                            ...
                            self->pointAnnotation_1.title = kJL_TXT("最后定位时间：现在");
                        ...
                        if(self.deviceObjc.type == 1){ //耳机
                            if(self->entity.mPower_L>0 && self->entity.mPower_R>0){ //左右耳有电量，显示一对耳机的位置
                                ...
                                self->pointAnnotation_2.title = kJL_TXT("最后定位时间：现在");
                            ...
                            if(self->entity.mPower_L>0 && self->entity.mPower_R == 0){ //左耳有电量，右耳电量为0(断连)
                                ...
                                for (int i = 0; i < 2; ++i)
                                {
                                    ...
                                    if(i == 0){
                                        mPointAnnotation.title      = kJL_TXT("最后定位时间：现在");
                                    }
                                    if(i == 1){
                                        DeviceObjc *objc = [[SqliteManager sharedInstance] checkoutByUuid:self.deviceObjc.uuid];
                                        mPointAnnotation.title = [NSString stringWithFormat:@"%@%@",kJL_TXT("最后定位时间:"),[self timeFromNow:objc.rightLastTime Type:1]];
                                    }
                            ...
                            if(self->entity.mPower_R>0 && self->entity.mPower_L == 0){ //右耳有电量，左耳电量为0(断连)
                                ...
                                for (int i = 0; i < 2; ++i)
                                {
                                    ...
                                    if(i == 0){
                                        DeviceObjc *objc = [[SqliteManager sharedInstance] checkoutByUuid:self.deviceObjc.uuid];
                                        mPointAnnotation.title = [NSString stringWithFormat:@"%@%@",kJL_TXT("最后定位时间:"),[self timeFromNow:objc.leftLastTime Type:1]];
                                    }
                                    if(i == 1){
                                        mPointAnnotation.title = kJL_TXT("最后定位时间：现在");
                                    }
                        ...
                        if(![self->locationLab.text isEqualToString:@"正在定位中..."]){
        ...
        titleLab.text = _deviceObjc.name;//kJL_TXT("设备定位");
        ...
        if(self.deviceObjc.type == 0 || self.deviceObjc.type == 4){
            ...
            self->pointAnnotation_3.title = [NSString stringWithFormat:@"%@%@",kJL_TXT("最后定位时间:"),[self timeFromNow:self.deviceObjc.lastTime Type:1]];
        ...
        if(self.deviceObjc.type == 1){ //耳机
            if(entity.mPower_L>0 && entity.mPower_R>0){ //左右耳有电量，显示一对耳机的位置
                ...
                self->pointAnnotation_4.title = [NSString stringWithFormat:@"%@%@",kJL_TXT("最后定位时间:"),[self timeFromNow:self.deviceObjc.lastTime Type:1]];
...
- (IBAction)stopVoiceBtnAction:(id)sender {
    ...
    [self stopSearch];
    [[[JL_RunSDK sharedMe] mBleEntityM].mCmdManager cmdFindDevice:NO timeOut:60 findIphone:NO Operation:nil];
...
-(void)initWithUI{
    ...
    if ([CLLocationManager locationServicesEnabled]      &&
        ...
        //定位不能用
        [DFUITools showText_1:kJL_TXT("没有授权定位，请到后台设置...") onView:self.view delay:4];
    ...
    [searchBtn setTitle:kJL_TXT("查找设备") forState:UIControlStateNormal];
    [closeVoiceBtn setTitle:kJL_TXT("关闭声音") forState:UIControlStateNormal];
    ...
    tipsView.title = kJL_TXT("提示");
    tipsView.message = kJL_TXT("查找设备时，耳机将持续播放一段响铃，请确保它们均已从双耳摘下在继续。继续佩戴可能会因强音量感到不适");
        ...
        if (isLoad) {
            if([dict[@"POWER_L"] intValue] != self->entity.mPower_L
                ...
                if (uuidType == JLUuidTypeDisconnected||
                    ...
                    self->leftLabel.text = kJL_TXT("未连接");
                    ...
                    self->rightLabel.text = kJL_TXT("未连接");
                    ...
                    if(self->entity.mPower_L > 0){
                        if((self->playWayArray.count == 2 && [self->playWayArray containsObject:@(1)]
                           && [self->playWayArray containsObject:@(2)]) || (self->playWayArray.count == 1 && [self->playWayArray containsObject:@(1)])){
                            ...
                            self->leftLabel.text = kJL_TXT("已连接");
                        ...
                        if((self->playWayArray.count == 0) ||
                           (self->playWayArray.count == 1 && [self->playWayArray containsObject:@(2)])){
                            ...
                            self->playSoundLeftLabel.frame = CGRectMake(17.5,59.5,100,14);
                            self->playSoundLeftLabel.numberOfLines = 0;
                            [self->bottomVoicesLeftView addSubview:self->playSoundLeftLabel];
                            self->playSoundLeftLabel.hidden = NO;

                            NSMutableAttributedString *playSoundLeftStr = [[NSMutableAttributedString alloc] initWithString:kJL_TXT("播放声音") attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 15],NSForegroundColorAttributeName: kColor_0000}];
                    ...
                    if(self->entity.mPower_R >0){
                        if((self->playWayArray.count == 2 && [self->playWayArray containsObject:@(1)]
                           && [self->playWayArray containsObject:@(2)]) || (self->playWayArray.count == 1 && [self->playWayArray containsObject:@(2)])){
                            ...
                            self->rightLabel.text = kJL_TXT("已连接");
                        ...
                        if((self->playWayArray.count == 0) ||
                           (self->playWayArray.count == 1 && [self->playWayArray containsObject:@(1)])){
                            ...
                            self->playSoundRightLabel.frame = CGRectMake(17.5,59.5,100,14);
                            self->playSoundRightLabel.numberOfLines = 0;
                            [self->bottomVoicesRightView addSubview:self->playSoundRightLabel];
                            self->playSoundRightLabel.hidden = NO;

                            NSMutableAttributedString *playSoundRightStr = [[NSMutableAttributedString alloc] initWithString:kJL_TXT("播放声音") attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 15],NSForegroundColorAttributeName: kColor_0000}];
...
-(void)startSearch{
    ...
    if (![addr isEqualToString:edr]) {
        [DFUITools showText:kJL_TXT("请连设备蓝牙!") onView:self.view delay:1.0];
        return ;
    }
    
    if(self.deviceObjc.type == 0 || self.deviceObjc.type == 4){
        [[[JL_RunSDK sharedMe] mBleEntityM].mCmdManager cmdFindDevice:YES timeOut:60 findIphone:NO Operation:nil];
    }
    ...
    if(self.deviceObjc.type == 1){
        ...
        //发送当前命令给固件
        [[[JL_RunSDK sharedMe] mBleEntityM].mCmdManager cmdFindDevice:YES timeOut:60 findIphone:NO Operation:dictWay];
        //请求当前的位置
        [self findDevice];
        [self showSoundUI];
...
-(void)voiceDidSelectWith:(NSInteger)index{
    ...
    if (index == 1) {
        if(self.deviceObjc.type == 1 && tipsView.hidden == NO){
...
-(void)calcuteWithDistance:(CLLocationCoordinate2D)coordinate_0 with:(CLLocationCoordinate2D)coordinate_1{
    ...
    distanceLab.text = [NSString stringWithFormat:@"%.1f%@",distance_1,kJL_TXT("公里")];
}
...
-(NSString *)timeFromNow:(NSDate *)date Type:(int) type{
    ...
    if(type == 0){
        if ((d1-d2)>=86400) {
            str = [NSString stringWithFormat:@"%@%@%@",kJL_TXT("昨天"),[formatter stringFromDate:date],kJL_TXT("分")];
            if ((d1-d2) >= 86400*2){
                str = [NSString stringWithFormat:@"%@%@%@",kJL_TXT("前天"),[formatter stringFromDate:date],kJL_TXT("分")];
                if ((d1-d2) >= 86400*3){
                    str = [NSString stringWithFormat:@"%@%@%@",kJL_TXT("两天前"),[formatter stringFromDate:date],kJL_TXT("分")];
                    if ((d1-d2) >= 86400*4){
            ...
            if([kJL_GET hasPrefix:@"en"]){
                str = [NSString stringWithFormat:@"%@ %@ %@",kJL_TXT("今天"),[formatter stringFromDate:date],kJL_TXT("分")];
            }
        ...
        if (bleSDK.mBleMultiple.bleManagerState == CBManagerStatePoweredOn) {
            if([kJL_GET hasPrefix:@"en"]){
                str = [NSString stringWithFormat:@"%@ %@ %@",kJL_TXT("今天"),[formatter stringFromDate:nowDate],kJL_TXT("分")];
            }
            if([kJL_GET hasPrefix:@"zh"]){
                str = [NSString stringWithFormat:@"%@%@%@",kJL_TXT("今天"),[formatter stringFromDate:nowDate],kJL_TXT("分")];
            }
...
//        pointAnnotation.title = @"最后定位时间：现在";
...
-(void)initBottomViews{
    ...
    if(self.deviceObjc.type == 1){ //1:耳机
        ...
        bottomsVociesView.frame = CGRectMake(24,[DFUITools screen_2_H]-115-28,[DFUITools screen_2_W]-24*2,115);
        ...
        bottomVoicesLeftView.frame = CGRectMake(0,0,([DFUITools screen_2_W]-24*2-17)/2,115);
        bottomVoicesLeftView.backgroundColor = [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1.0];
        ...
        UIButton *bottomVoicesLeftBtn = [[UIButton alloc] initWithFrame:CGRectMake(0,0,([DFUITools screen_2_W]-24*2-17)/2,115)];
        ...
        bottomVoicesRightView.frame = CGRectMake(bottomVoicesLeftView.frame.origin.x+bottomVoicesLeftView.frame.size.width+17,0,([DFUITools screen_2_W]-24*2-17)/2,115);
        bottomVoicesRightView.backgroundColor = [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1.0];
        ...
        UIButton *bottomVoicesRightBtn = [[UIButton alloc] initWithFrame:CGRectMake(bottomVoicesLeftView.frame.origin.x+bottomVoicesLeftView.frame.size.width+17,0,([DFUITools screen_2_W]-24*2-17)/2,115)];
        ...
        NSMutableAttributedString *leftLabelStr = [[NSMutableAttributedString alloc] initWithString:kJL_TXT("已连接") attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 15],NSForegroundColorAttributeName: [UIColor colorWithRed:36/255.0 green:36/255.0 blue:36/255.0 alpha:1.0]}];

        leftLabel.attributedText = leftLabelStr;
        
        playSoundLeftLabel = [[UILabel alloc] init];
        playSoundLeftLabel.frame = CGRectMake(17.5,59.5,100,14);
        ...
        NSMutableAttributedString *rightLabelStr = [[NSMutableAttributedString alloc] initWithString:kJL_TXT("已连接") attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 15],NSForegroundColorAttributeName: [UIColor colorWithRed:36/255.0 green:36/255.0 blue:36/255.0 alpha:1.0]}];

        rightLabel.attributedText = rightLabelStr;
        
        playSoundRightLabel = [[UILabel alloc] init];
        playSoundRightLabel.frame = CGRectMake(17.5,59.5,100,14);
...
-(void)leftEvent:(UITapGestureRecognizer *)gesture
{
    ...
    if (uuidType == JLUuidTypeDisconnected
        || uuidType == JLUuidTypeConnected) {
        [DFUITools showText:kJL_TXT("左耳不能查找设备") onView:self.view delay:1.0];
        return;
    }else if(uuidType == JLUuidTypeInUse){
...
-(void)rightEvent:(UITapGestureRecognizer *)gesture
{
    ...
    if (uuidType == JLUuidTypeDisconnected
        || uuidType == JLUuidTypeConnected) {
        [DFUITools showText:kJL_TXT("右耳不能查找设备") onView:self.view delay:1.0];
        return;
    }
    else if(uuidType == JLUuidTypeInUse){
