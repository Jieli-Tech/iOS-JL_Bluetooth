...
-(void)notePlayerProgress:(NSNotification*)note{
    ...
    if (isID3_push == YES) {
        NSLog(@"UI关闭ID3信息。100");
        [entity.mCmdManager cmdID3_PushEnable:NO];
...
-(void)setID3_Enable:(BOOL)enable Action:(ID3_ACTION)action{
    ...
    if (isID3_push == !enable) {
        ...
        [DFAction delay:0.3 Task:^{
            [[[JL_RunSDK sharedMe] mBleEntityM].mCmdManager cmdID3_PushEnable:enable];
        }];
    }else{
        if (action) { action();}
    }
...
- (IBAction)actionLast:(id)sender {
    ...
    if(model.currentFunc == JL_FunctionCodeBT){
        ...
        if (isID3_ST == NO) {
            ...
            isWaitCutSong = YES;
            [entity.mCmdManager cmdID3_Before];
        }
    }
    if(model.currentFunc == JL_FunctionCodeMUSIC){
        [entity.mCmdManager cmdFunction:JL_FunctionCodeMUSIC
...
-(BOOL)isEdrOK{
    ...
    if (![addr isEqualToString:entity.mEdr]) {
        ...
        [DFUITools showText:kJL_TXT("请连对应设备的蓝牙!") onView:win delay:1.0];
        return NO;
    }
    return YES;
}
    ...
    if(model.currentFunc == JL_FunctionCodeBT){
        ...
        if (isID3_ST == NO) {
                ...
                nowPlayer.mState == DFAudioPlayer_PENDING)
            {
                ...
                [entity.mCmdManager cmdID3_PushEnable:YES];
                isWaitPuase = YES;
            ...
            if (model.ID3_Status == 1) { //播放
                [entity.mCmdManager setID3_Status:1];
            }else{ //暂停
                [entity.mCmdManager setID3_Status:0];
            }
            [[JLCacheBox cacheUuid:bleSDK.mBleUUID] setIsID3_PLAY:YES];

            [entity.mCmdManager cmdID3_PP];
        }
    }
    if(model.currentFunc == JL_FunctionCodeMUSIC){
...
-(void)noteNetPlayerPause:(NSNotification*)note{
    [self updatePhoneMusicInfo];
    [[[JL_RunSDK sharedMe] mBleEntityM].mCmdManager cmdID3_PushEnable:YES];
    isWaitPuase = YES;
}
...
- (IBAction)actionNext:(id)sender {
    ...
    if(model.currentFunc == JL_FunctionCodeBT){
        ...
        if (isID3_ST == NO) {
            ...
            isWaitCutSong = YES;
            [entity.mCmdManager cmdID3_Next];
        }
    }
    if(model.currentFunc == JL_FunctionCodeMUSIC){
        [entity.mCmdManager cmdFunction:JL_FunctionCodeMUSIC
...
//    progress_date = [NSDate new];
    ...
    if(model.currentFunc == JL_FunctionCodeMUSIC){
        ...
        [JL_Tools mainTask:^{
            ...
            if(pg-current<0){
                [entity.mCmdManager cmdFastPlay:JL_FCmdMusicFastBack
            ...
            if(pg-current>0){
                [entity.mCmdManager cmdFastPlay:JL_FCmdMusicFastPlay
...
-(void)updateID3MusicStatus:(NSNotification*)note{
    ...
    if (deviceModel.ID3_Status == 1) { //播放
        ...
        if ((str_0 == 0 || str_0 == 1) &&
            ...
            (str_2 == 0 || str_2 == 1) &&
            time_0 == 0) {
            _mSonyName_1.text = @"ID3信息获取失败";
            //[DFUITools showText:@"ID3信息获取失败" onView:self delay:2.0];
        }
...
-(void)notePlayerInterruption:(NSNotification*)note{
    ...
    if (nowPlayer.mState == DFAudio_ST_PLAYING) {
        ...
        if(model.mCallType == JL_CALLType_OFF){
            ...
            [bleSDK.mBleEntityM.mCmdManager cmdID3_PushEnable:YES];
        }
...
-(void)noteEdrChange:(NSNotification*)note{
    ...
    if (dict == nil || ![edr isEqual:bleSDK.mBleEntityM.mEdr]) {
        ...
        //UIWindow *win = [DFUITools getWindow];
        [JL_Tools mainTask:^{
            [DFUITools showText:@"请连对应设备的蓝牙!" onView:self delay:1.0];
        }];
...
-(void)addNote{
    ...
    [JL_Tools add:JL_BLE_CHANGE_MASTER Action:@selector(noteBleChangeMaster:) Own:self];
