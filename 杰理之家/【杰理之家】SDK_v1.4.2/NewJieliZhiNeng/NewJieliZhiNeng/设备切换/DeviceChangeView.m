...
- (instancetype)init
{
    ...
    if (self) {
        ...
        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:kJL_TXT("我的设备") attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang SC" size: 18],NSForegroundColorAttributeName: [UIColor colorWithRed:36/255.0 green:36/255.0 blue:36/255.0 alpha:1.0]}];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.mImageView.image = [self getProductImageUUID:model.uuid Type:model.type]; 
...
-(void)onDeviceInfoBtnTag:(NSInteger)tag{
    ...
    if (type == JLUuidTypeConnected ) {
        UIWindow *win = [DFUITools getWindow];
        [DFUITools showText:kJL_TXT("此设备不是当前使用设备") onView:win delay:1.0];
        return;
    }
}
