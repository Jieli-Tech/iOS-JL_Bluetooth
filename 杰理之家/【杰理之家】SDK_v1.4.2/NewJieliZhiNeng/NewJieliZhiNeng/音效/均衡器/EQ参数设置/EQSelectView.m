...
@interface EQSelectView()<UITableViewDelegate,UITableViewDataSource>{
    NSArray     *dataArray;
    UIView      *deviceView;
    UIView      *blackView;
    UIView      *touchView;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        label.frame = CGRectMake(sW/2-50/2,0,100,50);
        ...
        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:kJL_TXT("预设") attributes:aDic];
        label.attributedText = string;
        
        UIView *sepView = [[UIView alloc] init];
        sepView.frame = CGRectMake(0,label.frame.origin.y+label.frame.size.height,sW,0.5);
        ...
        UIButton *btnOK = [[UIButton alloc] initWithFrame:rect_btn_0];
        btnOK.backgroundColor = [UIColor whiteColor];
        [btnOK addTarget:self action:@selector(onOkBtn) forControlEvents:UIControlEventTouchUpInside];
        btnOK.titleLabel.text = kJL_TXT("取消");
        [btnOK setTitle:kJL_TXT("取消") forState:UIControlStateNormal];
        [btnOK setTitleColor:kDF_RGBA(72, 72, 72, 1.0) forState:UIControlStateNormal];
        btnOK.titleLabel.font = [UIFont fontWithName:@"PingFang SC" size:15];
        [deviceView addSubview:btnOK];
        [self updateUI];
    }
    return self;
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (eqBlk) {
        ...
        if (model.eqType == JL_EQType10) {
            ...
            NSArray *eqDefaultArr = model.eqDefaultArray;
            for (JLEQModel *eqModel in eqDefaultArr) {
