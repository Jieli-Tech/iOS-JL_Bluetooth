...
-(void)setupUI{
    ...
    titleName.text = kJL_TXT("网络电台");
    [btnEdit setTitle:kJL_TXT("管理") forState:UIControlStateNormal];
    
    [btn1 setTitle:kJL_TXT("本地电台") forState:UIControlStateNormal];
    [btn2 setTitle:kJL_TXT("国家电台") forState:UIControlStateNormal];
    [btn3 setTitle:kJL_TXT("省市电台") forState:UIControlStateNormal];
    [btn4 setTitle:kJL_TXT("我的收藏") forState:UIControlStateNormal];
}
...
-(void)requestHttpData{
    ...
    [self startLoadingView:@"搜索电台..." Delay:10.0];
    [JL_Tools subTask:^{
        /*--- 省份ID ---*/
...
- (IBAction)btn_place:(id)sender {
    if (netPlayer.provinceIDArray.count == 0) {
        [DFUITools showText:@"未找到省份." onView:self.view delay:1.0];
        return;
    }
...
- (IBAction)btn_isEdit:(id)sender {
    if (isEdit) {
        ...
        isEdit = YES;
        [btnEdit setTitle:kJL_TXT("完成") forState:UIControlStateNormal];
    }
...
-(void)startLoadingView:(NSString*)text Delay:(NSTimeInterval)delay{
    ...
    [DFAction delay:delay-1.0 Task:^{
        self->loadingTp.labelText = @"未找到电台";
    }];
}
