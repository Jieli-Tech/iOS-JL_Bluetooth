...
-(void)updateMultiMediaUI{
    ...
    uint32_t fun_rtc      = function>>2&0x01;
    uint32_t fun_linein   = function>>3&0x01;
    uint32_t fun_fm       = function>>4&0x01;
    uint32_t fun_light    = function>>5&0x01;
    uint32_t fun_fmtx     = function>>6&0x01;
    uint32_t fun_find_dev = 1;
    uint32_t fun_net_radio= 1;
    uint32_t fun_karaoke  = 1;
    uint32_t fun_watch    = 0;//1;
    ...
    NSArray *enArr = @[@(fun_bt),@(fun_usb),@(fun_sd),
                       ...
                       @(fun_net_radio),@(fun_karaoke),@(fun_watch)];
