...
-(BOOL)updateLocate:(NSData *) local ForUUID:(NSString *)uuid{
    ...
    if ([db open]) {
        ...
        [db close];
        if (result) {
            //NSLog(@"update updateLocate OK");
            return YES;
        }else{
            NSLog(@"update  updateLocate failed");
            return NO;
        }
    }
    return NO;;
...
-(BOOL)updateLeftLastTime:(NSDate *)leftLastTime ForUUID:(NSString *)uuid{
    if ([db open]) {
        ...
        [db close];
        if (result) {
            //NSLog(@"update updateLeftLastTime OK");
            return YES;
        }else{
            NSLog(@"update  updateLeftLastTime failed");
            return NO;
        }
    }
    return NO;;
...
-(BOOL)updateRightLastTime:(NSDate *)rightLastTime ForUUID:(NSString *)uuid{
    if ([db open]) {
        ...
        [db close];
        if (result) {
            //NSLog(@"update updateRightLastTime OK");
            return YES;
        }else{
            NSLog(@"update  updateRightLastTime failed");
            return NO;
        }
    }
    return NO;;
...
-(BOOL)updateLastTime:(NSDate *)lastTime ForUUID:(NSString *)uuid{
    if ([db open]) {
        ...
        [db close];
        if (result) {
            //NSLog(@"update updateLastTime OK");
            return YES;
        }else{
            NSLog(@"update  updateLastTime failed");
            return NO;
        }
    }
    return NO;;
...
-(BOOL)updateLastAddress:(NSString *)addr ForUUID:(NSString *)uuid{
    if ([db open]) {
          ...
          [db close];
          if (result) {
              //NSLog(@"update address OK");
              return YES;
          }else{
              NSLog(@"update  address failed");
              return NO;
          }
      }
      return NO;
...
-(BOOL)updateLastFindDeviceEnable:(BOOL)enable ForUUID:(NSString *)uuid{
    if ([db open]) {
        ...
        [db close];
        if (result) {
            //NSLog(@"update findDevice OK");
            return YES;
        }else{
            NSLog(@"update  findDevice failed");
            return NO;
        }
    }
    return NO;
...
-(BOOL)installWithDevice:(JL_EntityM *)entity{
    ...
    //NSLog(@"%s:%d name:%@",__func__,entity.mType,entity.mItem);
    
    if ([db open]) {
        NSString *type = [NSString stringWithFormat:@"%d",entity.mType];
        BOOL result = [db executeUpdate:@"INSERT INTO t_devices (name, uuid ,type,uid,pid,edr) VALUES (?, ? ,?,?,?,?);",entity.mItem,entity.mPeripheral.identifier.UUIDString,type,entity.mVID,entity.mPID,entity.mEdr];
        [db close];
...
-(BOOL)updateDeviceBy:(JL_EntityM *)entity{
    ...
    //NSLog(@"%s:%d name:%@",__func__,entity.mType,entity.mItem);
    NSString *sql = [NSString stringWithFormat:@"UPDATE t_devices set name = '%@', uuid = '%@', type = '%d', uid = '%@' , pid = '%@' ,edr = '%@' where uuid = '%@'",entity.mItem,entity.mPeripheral.identifier.UUIDString,entity.mType,entity.mVID,entity.mPID,entity.mEdr,entity.mPeripheral.identifier.UUIDString];
      if ([db open]) {
          BOOL result = [db executeUpdate:sql];
          [db close];
          if (result) {
...
-(BOOL)updateDeviceByObjc:(DeviceObjc *)entity{
    ...
    //NSLog(@"%s:%d name:%@",__func__,entity.type,entity.name);
    NSString *sql = [NSString stringWithFormat:@"UPDATE t_devices set name = '%@', uuid = '%@', type = '%d' , uid = '%@' , pid = '%@' ,edr = '%@' where uuid = '%@'",entity.name,entity.uuid,entity.type,entity.uid,entity.pid,entity.edr,entity.uuid];
      if ([db open]) {
          BOOL result = [db executeUpdate:sql];
          [db close];
          if (result) {
...
-(void)checkWetherShouledNewOne{
    ...
    if (![db columnExists:@"findDevice" inTableWithName:@"t_devices"]) {
        ...
        if(worked){
            NSLog(@"findDevice 插入成功");
        }else{
            NSLog(@"findDevice 插入失败");
        }
    }
    ...
    if (![db columnExists:@"leftLastTime" inTableWithName:@"t_devices"]) {
        ...
        BOOL worked = [db executeUpdate:alertStr];
        if(worked){
            NSLog(@"leftLastTime 插入成功");
        }else{
            NSLog(@"leftLastTime 插入失败");
        }
    }
    if (![db columnExists:@"rightLastTime" inTableWithName:@"t_devices"]) {
        ...
        BOOL worked = [db executeUpdate:alertStr];
        if(worked){
            NSLog(@"rightLastTime 插入成功");
        }else{
            NSLog(@"rightLastTime 插入失败");
        }
    }
    [db close];
}
...
-(void)updateDeviceModel:(JLModel_Device *)model forUUID:(NSString *)uuid{
    ...
    if ([db open]) {
        ...
        if (array.count==1) {
            ...
            if (result) {
                //NSLog(@"authkey proCode update OK");
            }else{
                NSLog(@"authkey proCode update Failed");
            }
        }else{
            BOOL result = [db executeUpdate:@"INSERT INTO t_device_model (uuid ,authkey,proCode) VALUES (?, ? ,?);",uuid,model.authKey,model.proCode];
            if (result) {
                NSLog(@"authkey proCode install OK");
            }else{
                NSLog(@"authkey proCode install Failed");
            }
        }
...
-(void)deleteDeviceModelByUUid:(NSString *)uuid{
    if ([db open]) {
        ...
        BOOL result = [db executeUpdate:sql];
        if (result) {
            NSLog(@"delete t_device_model by uuid OK");
        }else{
            NSLog(@"delete t_device_model by uuid failed");
        }
        [db close];
    }
}
